  ...
  int offset, emxArray_real_T *b_y1, emxArray_real_T *y2)
{
  ...
  if ((M->data[lvl - 1].L.val->size[0] == 0) && (M->data[lvl - 1].U.val->size[0]
    ...
    for (b_n = 1; b_n <= nB; b_n++) {
      k += b_n;
      for (i = b_n; i + 1 <= nB; i++) {
        b_y1->data[i] -= M->data[lvl - 1].U.val->data[k] * b_y1->data[b_n - 1];
        k++;
      }
    }

    k = M->data[lvl - 1].L.nrows * M->data[lvl - 1].L.nrows - 1;
    for (b_n = M->data[lvl - 1].L.nrows - 1; b_n + 1 > 0; b_n--) {
      b_y1->data[b_n] /= M->data[lvl - 1].U.val->data[k];
      for (i = b_n; i > 0; i--) {
        k--;
        b_y1->data[i - 1] -= M->data[lvl - 1].U.val->data[k] * b_y1->data[b_n];
      }

      k = ((k - nB) + b_n) - 1;
    }
  } else {
    b_n = M->data[lvl - 1].L.row_ptr->size[0] - 1;
    for (i = 0; i + 1 <= b_n; i++) {
      for (k = M->data[lvl - 1].L.row_ptr->data[i]; k < M->data[lvl - 1].
           L.row_ptr->data[i + 1]; k++) {
        b_y1->data[i] -= M->data[lvl - 1].L.val->data[k - 1] * b_y1->data
          [M->data[lvl - 1].L.col_ind->data[k - 1] - 1];
      }
    }
    ...
    for (i = M->data[lvl - 1].U.row_ptr->size[0] - 2; i + 1 > 0; i--) {
      for (b_n = M->data[lvl - 1].U.row_ptr->data[i]; b_n < M->data[lvl - 1].
           U.row_ptr->data[i + 1]; b_n++) {
        b_y1->data[i] -= M->data[lvl - 1].U.val->data[b_n - 1] * b_y1->data
          [M->data[lvl - 1].U.col_ind->data[b_n - 1] - 1];
      }
    }
  }
