 ...
 * fgmresMILU_GS_mex.c
 *
 * Auxiliary code for mexFunction of fgmresMILU_GS
 *
...
static void __fgmresMILU_GS_api(mxArray **plhs, const mxArray ** prhs) {
    struct0_T            A;
    emxArray_real_T      b;
    struct1_T            prec;
    int32_T              restart;
    ...
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxSTRUCT_CLASS)
        mexErrMsgIdAndTxt("fgmresMILU_GS:WrongInputType",
            "Input argument A has incorrect data type; struct is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("fgmresMILU_GS:WrongSizeOfInputArg",
    ...
    fgmresMILU_GS(&A, &b, &prec, restart, rtol, maxit, &x0, verbose, nthreads, &param, &rowscal, &colscal, &x, flag, iter, &resids);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 12) {
        if (nlhs > 4)
            mexErrMsgIdAndTxt("fgmresMILU_GS:TooManyOutputArguments",
                "Too many output arguments for entry-point fgmresMILU_GS.\n");
        /* Call the API function. */
        __fgmresMILU_GS_api(outputs, prhs);
    }
    else
        mexErrMsgIdAndTxt("fgmresMILU_GS:WrongNumberOfInputs",
            "Incorrect number of input variables for entry-point fgmresMILU_GS.");
