...
void gmresMILU_MGS(const struct0_T *A, const emxArray_real_T *b, const struct1_T
    ...
    while ((!exitg1) && (it_outer <= max_outer_iters)) {
      ...
      do {
        ...
        for (ii = 0; ii + 1 <= j + 1; ii++) {
          ...
          if (a->size[1] == 1) {
            ...
            if (i0 == 1) {
              ...
              for (i0 = 0; i0 < a->size[1]; i0++) {
                beta2 += a->data[a->size[0] * i0] * b_b->data[i0];
              }
            }
          }
          ...
          loop_ub = v->size[0];
          for (i0 = 0; i0 < loop_ub; i0++) {
            v->data[i0] -= beta2 * Q->data[i0 + Q->size[0] * ii];
          }
        }
        ...
        beta2 = sqrt(w->data[j] * w->data[j] + beta2);
        J->data[J->size[0] * j] = w->data[j] / beta2;
      ...
      for (ii = 0; ii + 1 <= j + 1; ii++) {
        ...
        loop_ub = x->size[0];
        for (i0 = 0; i0 < loop_ub; i0++) {
          x->data[i0] += beta2 * Z->data[i0 + Z->size[0] * ii];
        }
      }
...
void gmresMILU_MGS_initialize(void)
{
}

void gmresMILU_MGS_terminate(void)
{
}
