...
m2mat=0;
if nargin>=6
   if isstr(M2) || isa(M2,'function_handle')
      m2func=1;
   end
   if ~m2func
      if size(M2,2)==n
...
m1mat=0;
if nargin>=5
   if isstr(M1) || isa(M1,'function_handle')
      m1func=1;
   end
   if ~m1func
      if size(M1,2)==n
...
if (m2func || m2mat) && ~m1mat && ~m1funct
   error('preconditioning matrices M1, M2 are invalid');
end
...
if isreal(A)
   if norm(A-A',1)==0
      % we cannot check preconditioner seriously
      % complex-symmetric preconditioner by default
      if m1func || m2func
	 choice=3;
      else
	 % preconditioner available as a matrix
	 % real case, M1 and M2 are available as matrices
	 if m1mat && m2mat
	    if isreal(M1) & isreal(M2)
	       choice=1;
	    else % complex case
	       choice=3;
	    end
	 % only M1 is available as matrix
	 elseif m1mat
	    if isreal(M1)
	       choice=1;
	    else % complex case
	 ...
	 else % no preconditioning, A is real
	    choice=1;
	 end % if
      end
   end
...
while flag
	 ...
	 % initially M==I, no preconditioning
         drain=src;
         % check if M=M2 is passed
	 % Then M1 must be passed, too
	 if m2mat || m2func
	    % drain=M2\(M1\src);
  	    if isstr(M1)
	       drain=feval(M1,src);
	    elseif isa(M1,'function_handle')
  	    ...
  	    if isstr(M2)
	       drain=feval(M2,drain);
	    elseif isa(M2,'function_handle')
	       drain=M2(drain);
   	    else
	       drain=M2\drain;
            end
         % check if at least M=M1 is passed
         elseif m1mat || m1func
