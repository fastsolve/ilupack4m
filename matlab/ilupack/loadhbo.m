...
if length(filename)>=3
   if filename(end-2)=='.' && filename(end-1)=='r' && filename(end)=='b'
      fp=fopen(filename);
      if fp~=-1
	 tline=fgetl(fp);
	 tline=fgetl(fp);
...
% filenames to be completed by '.rsa', '.rua',...
