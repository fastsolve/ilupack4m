/* $Id: ZHERildlfactor.c 810 2016-02-23 05:34:54Z bolle $ */
/* ========================================================================== */
...
/*
    ...
    [L,D,P,scal,options]=ZHERildlfactor(A,options)
...
)
{
    ...
    ZHERAMGinit(&A,&options);
#ifdef PRINT_INFO
    mexPrintf("ZHERildlfactor: sparse matrix imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZHERildlfactor: options structure imported\n");fflush(stdout);
#endif



    /* memory for scaling and permutation */
    p   =(integer *)MAlloc((size_t)A.nc*sizeof(integer),"ZHERildlfactor");
    invq=(integer *)MAlloc((size_t)A.nc*sizeof(integer),"ZHERildlfactor");
    pcolscale=(doublecomplex *)MAlloc((size_t)A.nc*sizeof(doublecomplex),"ZHERildlfactor");
    prowscale=pcolscale;
    nB=A.nc;
    if (options.matching) {
	  ...
	  if (options.ind==NULL)
	     ierr=ZHERperm_mc64_metis_e(A, prowscale, pcolscale, p, invq, &nB, &options);
	  else
	     ierr=ZHERperm_mc64_metis_e_sp(A, prowscale, pcolscale, p, invq, &nB, &options);
#else
	  ierr=ZHERperm_matching_metis_e(A, prowscale, pcolscale, p, invq, &nB, &options);
#endif
#ifdef PRINT_INFO
	  mexPrintf("ZHERildlfactor: mwm+metise performed\n");fflush(stdout);
#endif
       }
       else if (!strcmp("amd",options.ordering)) {
#ifdef _MC64_MATCHING_    
	  if (options.ind==NULL)
	     ierr=ZHERperm_mc64_amd(A, prowscale, pcolscale, p, invq, &nB, &options);
	  else
	     ierr=ZHERperm_mc64_amd_sp(A, prowscale, pcolscale, p, invq, &nB, &options);
#else
	  ierr=ZHERperm_matching_amd(A, prowscale, pcolscale, p, invq, &nB, &options);
#endif
#ifdef PRINT_INFO
	  mexPrintf("ZHERildlfactor: mwm+amd performed\n");fflush(stdout);
#endif
       }
       else if (!strcmp("rcm",options.ordering)) {
#ifdef _MC64_MATCHING_    
	  if (options.ind==NULL)
	     ierr=ZHERperm_mc64_rcm(A, prowscale, pcolscale, p, invq, &nB, &options);
	  else
	     ierr=ZHERperm_mc64_rcm_sp(A, prowscale, pcolscale, p, invq, &nB, &options);
#else
	  ierr=ZHERperm_matching_rcm(A, prowscale, pcolscale, p, invq, &nB, &options);
#endif
#ifdef PRINT_INFO
	  mexPrintf("ZHERildlfactor: mwm+rcm performed\n");fflush(stdout);
#endif
       }
...
#ifdef _MC64_MATCHING_    
	  if (options.ind==NULL)
	     ierr=ZHERperm_mc64_metis_n(A, prowscale, pcolscale, p, invq, &nB, &options);
	  else
	     ierr=ZHERperm_mc64_metis_n_sp(A, prowscale, pcolscale, p, invq, &nB, &options);
#else
	  ierr=ZHERperm_matching_metis_n(A, prowscale, pcolscale, p, invq, &nB, &options);
#endif
#ifdef PRINT_INFO
	  mexPrintf("ZHERildlfactor: mwm+metisn performed\n");fflush(stdout);
#endif
       }
    }
    else {
       if (!strcmp("metisn",options.ordering)) {
	  ierr=ZHERperm_metis_n(A, prowscale, pcolscale, p, invq, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("ZHERildlfactor: metisn performed\n");fflush(stdout);
#endif
       }
       else if (!strcmp("metise",options.ordering)) {
	  ierr=ZHERperm_metis_e(A, prowscale, pcolscale, p, invq, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("ZHERildlfactor: metise performed\n");fflush(stdout);
#endif
       }
       else if (!strcmp("amd",options.ordering)) {
	  ierr=ZHERperm_amd(A, prowscale, pcolscale, p, invq, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("ZHERildlfactor: amd performed\n");fflush(stdout);
#endif
       }
       else if (!strcmp("rcm",options.ordering)) {
	  ierr=ZHERperm_rcm(A, prowscale, pcolscale, p, invq, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("ZHERildlfactor: rcm performed\n");fflush(stdout);
#endif
       }
       else { /* none */
	  ierr=ZHERperm_null(A, prowscale, pcolscale, p, invq, &nB, &options);
#ifdef PRINT_INFO
	  mexPrintf("ZHERildlfactor: only scaling performed\n");fflush(stdout);
#endif
       }
    }
#ifdef PRINT_INFO
    mexPrintf("ZHERildlfactor: scaling and reordering computed, ierr=%d\n",ierr);fflush(stdout);
#endif

#ifdef PRINT_INFO
    mexPrintf("ZHERildlfactor: matrix rescaled\n");fflush(stdout);
#endif
#ifdef PRINT_INFO2
    mexPrintf("p\n");
    for (i=0; i<A.nr; i++) {
    ...
    /* auxiliary buffers */
    ibuff=(integer *)      MAlloc((size_t)14*A.nc*sizeof(integer),      "ZHERildlfactor");
    dbuff=(doublecomplex *)MAlloc((size_t) 4*A.nc*sizeof(doublecomplex),"ZHERildlfactor");

    /* ilu buffers */
    /* just a simple guess at least 2n+1! */
    mem=ELBOW*A.ia[A.nr]+1;
    jlu=(integer *)      MAlloc((size_t)mem*sizeof(integer),      "ZHERildlfactor");
    alu=(doublecomplex *)MAlloc((size_t)mem*sizeof(doublecomplex),"ZHERildlfactor");
...
#endif
	  jlu=(integer *)      ReAlloc(jlu,mem*sizeof(integer),      "ZHERildlfactor:jlu");
	  alu=(doublecomplex *)ReAlloc(alu,mem*sizeof(doublecomplex),"ZHERildlfactor:alu");
#ifdef PRINT_INFO
...
#ifdef PRINT_INFO
      mexPrintf("ZHERildlfactor: matrix factorized, ierr=%d\n",ierr);fflush(stdout);
#endif
...
#ifdef PRINT_INFO
      mexPrintf("ZHERildlfactor: L requires %d nnz\n",nnz);fflush(stdout);
#endif
    ...
    while (i<A.nc) {
          ...
          if (jlu[A.nc+1+i]==0) {
#ifdef PRINT_INFO2
	     mexPrintf("ZHERildlfactor: row %d\n",i+1);fflush(stdout);
#endif
             /* diagonal entry */
             /* index */
             A_ja[k]=i;
		 ...
		 A_valuesR[k]  = alu[j-1].r*alu[i].r;
		 A_valuesI[k++]=-alu[j-1].i*alu[i].r;
             } /* end for j */
...
#ifdef PRINT_INFO2
	     mexPrintf("ZHERildlfactor: rows %d:%d\n",i+1,i+2);fflush(stdout);
#endif
	     ...
	     */
		 ...
		 A_valuesR[k]  = alu[m-1].r*alu[i].r       
		                +alu[m].r  *alu[A.nc+1+i].r-alu[m].i  *alu[A.nc+1+i].i;
		 A_valuesI[k++]=                           -alu[m-1].i*alu[i].r
				-alu[m].r  *alu[A.nc+1+i].i-alu[m].i  *alu[A.nc+1+i].r;
		 m+=2;
             } /* end for j */
		 ...
		 A_valuesR[k]  = alu[m-1].r*alu[A.nc+1+i].r+alu[m-1].i*alu[A.nc+1+i].i
		                +alu[m].r  *alu[i+1].r;
		 A_valuesI[k++]= alu[m-1].r*alu[A.nc+1+i].i-alu[m-1].i*alu[A.nc+1+i].r
		                                           -alu[m].i  *alu[i+1].r;
		 m+=2;
             } /* end for j */
...
#ifdef PRINT_INFO
    mexPrintf("ZHERildlfactor: nnz consumed %d\n",k);fflush(stdout);
#endif
#ifdef PRINT_INFO2
    for (i=0; i<A.nc; i++) {
      mexPrintf("row %d\n", i+1);
...
#endif
#ifdef PRINT_INFO
      mexPrintf("ZHERildlfactor: L exported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZHERildlfactor: D requires %d\n",nnz);fflush(stdout);
#endif
    ...
    while (i<A.nc) {
	     ...
	     A_valuesI[k]=0.0;
             ...
             /* determinant */
             dbuf=alu[i].r*alu[i+1].r
	         -alu[A.nc+1+i].r*alu[A.nc+1+i].r-alu[A.nc+1+i].i*alu[A.nc+1+i].i;
             /* index */
             A_ja[k]  =i;
             A_ja[k+1]=i+1;
	     /* values */
	     A_valuesR[k]  = alu[i+1].r/dbuf;
	     A_valuesI[k]  = 0.0;
	     A_valuesR[k+1]=-alu[A.nc+1+i].r/dbuf;
	     A_valuesI[k+1]= alu[A.nc+1+i].i/dbuf;
	     ...
	     k+=2;
             ...
             A_ja[k+1]=i+1;
	     /* values */
	     A_valuesR[k]  =-alu[A.nc+1+i].r/dbuf;
	     A_valuesI[k]  =-alu[A.nc+1+i].i/dbuf;
	     A_valuesR[k+1]= alu[i].r/dbuf;
	     A_valuesI[k+1]= 0.0;
...
#ifdef PRINT_INFO
    mexPrintf("ZHERildlfactor: D consumed %d\n",k);fflush(stdout);
#endif
#ifdef PRINT_INFO2
    for (i=0; i<A.nc; i++) {
      mexPrintf("row %d\n", i+1);
...
#endif
#ifdef PRINT_INFO
      mexPrintf("ZHERildlfactor: D exported\n");fflush(stdout);
#endif
    ...
    A_ia[A.nc]=A.nc;
#ifdef PRINT_INFO
    mexPrintf("ZHERildlfactor: P exported\n");fflush(stdout);
#endif
    ...
    A_ia[A.nc]=A.nc;
#ifdef PRINT_INFO
      mexPrintf("ZHERildlfactor: S exported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
      mexPrintf("ZHERildlfactor: options structure exported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZHERildlfactor: memory released\n");fflush(stdout);
#endif
