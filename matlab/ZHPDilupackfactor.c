	  ...
	  nompparts=(mwSize)PRE->nompparts;
	   ...
	   for (jstruct=0; jstruct<nlev; jstruct++) {
		  ...
		  *pr=current->ispartial;
		  ...
		  if (PRE->issingle)
		     fout=mxCreateSparse((mwSize)scurrent->A.nr,(mwSize)scurrent->A.nc,nnz, mxCOMPLEX);
		  else
		     fout=mxCreateSparse((mwSize) current->A.nr,(mwSize) current->A.nc,nnz, mxCOMPLEX);
