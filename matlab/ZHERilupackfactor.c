...
void mexFunction(
  ...
  integer *ibuff, *istack, blocksize, ii, *Ibuff, jj, kk, ll;
  size_t sizebuf;
  ...
  */
  ...
  PRE =
      (ZAMGlevelmat *)MAlloc((size_t)sizeof(ZAMGlevelmat), "ZHERilupackfactor");
...
/* mexPrintf("matrix factored\n");fflush(stdout); */
  ...
  */
  convert =
      (double *)MAlloc((size_t)n * sizeof(double), "ZHERilupackfactor:convert");
      ...
      if (jstruct == PRE->nlev - 1 &&
        ...
        if (PRE->isblock) {
          ...
          Dbuff = (doublecomplex *)MAlloc(
              (size_t)2 * m * m * sizeof(doublecomplex), "ZHERilupackfactor");
          ...
          for (i = 0; i < ((PRE->issingle) ? scurrent->nB : current->nB);) {
            ...
            zhetrf("u", &blocksize, Dbuff, &blocksize, Ibuff, Dbuff + kk, &kk,
                   &ll, 1);
            if (ll < 0) {
              mexPrintf("_HETRF: argument %d has an illegal value", -ll);
              return;
            }
            if (ll > 0) {
            ...
            kk = blocksize * blocksize;
            zhetri("u", &blocksize, Dbuff, &blocksize, Ibuff, Dbuff + kk, &ll,
                   1);
            if (ll < 0) {
          ...
          if (PRE->issingle) {
            for (i = 0; i < scurrent->nB;) {
              ...
              if (scurrent->LU.ja[scurrent->nB + 1 + i] == 0) {
                ...
                deti = scurrent->LU.a[i].r * scurrent->LU.a[i + 1].i +
                       ...
                       scurrent->LU.a[scurrent->nB + 1 + i].i *
                           scurrent->LU.a[scurrent->nB + 1 + i]
                               .r; /* should be zero for a Hermitian matrix */
                ...
                si[k++] =
                    det * (detr * scurrent->LU.a[i + 1].i -
                           deti * scurrent->LU.a[i + 1].r); /* should be zero */
                ...
                si[k++] =
                    det * (detr * scurrent->LU.a[i].i -
                           deti * scurrent->LU.a[i].r); /* should be zero */
            ...
            for (i = 0; i < current->nB;) {
              ...
              if (current->LU.ja[current->nB + 1 + i] == 0) {
                ...
                si[k++] =
                    det * (detr * current->LU.a[i + 1].i -
                           deti * current->LU.a[i + 1].r); /* should be zero */
                ...
                si[k++] =
                    det * (detr * current->LU.a[i].i -
                           deti * current->LU.a[i].r); /* should be zero */
      ...
      if (jstruct == PRE->nlev - 1 &&
        ...
        for (i = 0; i < ((PRE->issingle) ? scurrent->nB : current->nB);) {
          ...
          if (((PRE->issingle) ? scurrent->LU.ia[i] : current->LU.ia[i]) > 0) {
            ...
            si[k++] =
                0.0; /* ((PRE->issingle)?scurrent->LU.a[j].i:current->LU.a[j].i);
                        */
            ...
            if (PRE->issingle) {
              ...
              si[k++] = 0.0; /* scurrent->LU.a[j+scurrent->nB-i].i; */
              j += 2 * (scurrent->nB - i) - 1;
            } else {
              sr[k] = current->LU.a[j + current->nB - i].r;
              si[k++] = 0.0; /* current->LU.a[j+current->nB-i].i; */
              j += 2 * (current->nB - i) - 1;
            }
            i += 2;
          }
        ...
        if (PRE->isblock) {
          ...
          for (i = 0; i < ((PRE->issingle) ? scurrent->nB : current->nB);) {
            ...
            zhetri("u", &blocksize, Dbuff, &blocksize, Ibuff, Dbuff + kk, &ll,
                   1);
            if (ll < 0) {
              mexPrintf("_HETRI: argument %d has an illegal value", -ll);
              return;
            }
            for (k = 0; k < blocksize; k++) {
              for (l = k + 1; l < blocksize; l++) {
