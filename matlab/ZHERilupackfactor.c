...
)
{
    ...
    const char *pnames[]= {"n","nB", "L","D","U", "E","F", "rowscal","colscal", "p","invq",
			   "param","ptr", "isreal","isdefinite","issymmetric","ishermitian", "issingle", "A_H", "errorL", "errorU", "errorS", "isblock", "indB", "LB"};
    ...
    size_t     sizebuf;
    double     dbuf, dbufr, dbufi, *A_valuesR,  *A_valuesI, *convert, *sr, *si, *pr, *pi,
               det, detr,deti;
		 ...
		 Bunch-Kaufman pivoting */
	      ...
	      while (i<((PRE->issingle)?scurrent->nB:current->nB)) {
		    if (((PRE->issingle)?scurrent->LU.ia[i]:current->LU.ia[i])>0) {
		       ...
		       k++;
		       while (l--) {
			     pr[j  ]= ((PRE->issingle)?scurrent->LU.a[k].r:  current->LU.a[k].r);
			     pi[j++]= ((PRE->issingle)?scurrent->LU.a[k++].i:current->LU.a[k++].i);
		       }
		       i++;
		    }
		    else {
	   ...
	   if (jstruct==PRE->nlev-1 &&
		 ...
		 Bunch-Kaufman pivoting */
		  ...
		  if (((PRE->issingle)?scurrent->LU.ia[i]:current->LU.ia[i])>0) {
		     ...
		     /* diagonal entry */
		     irs[k]=i;
		     sr[k]  =((PRE->issingle)?scurrent->LU.a[j].r:current->LU.a[j].r);
		     si[k++]=0.0; /* ((PRE->issingle)?scurrent->LU.a[j].i:current->LU.a[j].i); */
		     ...
		     /* sub-diagonal entry */
		     irs[k]=i+1;
		     sr[k]  =((PRE->issingle)?scurrent->LU.a[j+1].r:current->LU.a[j+1].r);
		     si[k++]=((PRE->issingle)?scurrent->LU.a[j+1].i:current->LU.a[j+1].i);
	...
	*pr=1;
	...
	for (j=0; j<i; j++) {
	    ...
	    /* mexPrintf("24. field `indB': transfer vector into cell array component %d\n", j);fflush(stdout);  */
