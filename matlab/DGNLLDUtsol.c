...
/*
   ...
   pivots
    ...
    z=DGNLLDUtsol(y,Pvec,Slvec,Srvec,L,iD,UT)
...
void mexFunction(
...
#ifdef PRINT_INFO
  mexPrintf("DGNLLDUtsol: input parameter y imported\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("DGNLLDUtsol: input parameter Pvec imported\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("DGNLLDUtsol: input parameter Slvec imported\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("DGNLLDUtsol: input parameter Srvec imported\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("DGNLLDUtsol: input parameter L imported\n");
  fflush(stdout);
#endif
  ...
  if (mrows != ncols || mrows != n) {
    mexErrMsgTxt("Sixth input must be a square matrix of same size as the "
                 "first matrix.");
  }
...
#ifdef PRINT_INFO
  mexPrintf("DGNLLDUtsol: input parameter iD imported\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("DGNLLDUtsol: input parameter UT imported\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("DGNLLDUtsol: output vector and buffer allocated\n");
  fflush(stdout);
#endif
  ...
  for (i = 0; i < n; i++) {
    ...
    j = pPvecr[i] - 1;
    dbuff[i] = pSrvecr[i] * pyr[j];
  } /* end for i */
#ifdef PRINT_INFO
  mexPrintf(
      "DGNLLDUtsol: permutation + transposed left diagonal scaling done\n");
  fflush(stdout);
#endif

  /* forward substitution with transposed unit upper triangular matrix */
  for (i = 0; i < n; i++) {
 ...
 * is 1 */
    ...
    for (k = UT_ia[i] + 1; k < UT_ia[i + 1]; k++) {
      /* row index */
      j = UT_ja[k];
      dbuff[j] -= UT_valuesR[k] * dbuff[i];
    } /* end for k */
  }   /* end for i */
#ifdef PRINT_INFO
  mexPrintf("DGNLLDUtsol: transposed forward substitution done\n");
  fflush(stdout);
#endif

  /* multiplication with a transposed block diagonal matrix with 1x1 and 2x2
   * pivots */
  i = 0;
  j = iD_ia[0];
  while (i < n) {
    ...
    if (k - j > 1) {
      ...
      if (l - k > 1) { /* 2x2 pivot with four nonzero elements */
        x = iD_valuesR[j] * dbuff[i] + iD_valuesR[j + 1] * dbuff[i + 1];
        y = iD_valuesR[k] * dbuff[i] + iD_valuesR[k + 1] * dbuff[i + 1];
      } else { /* 2x2 pivot with three nonzero elements [ a_{ii}   a_{i,i+1};
                                                         a_{i+1,i}    0     ] */
        x = iD_valuesR[j] * dbuff[i] + iD_valuesR[j + 1] * dbuff[i + 1];
        y = iD_valuesR[k] * dbuff[i];
      }
      dbuff[i] = x;
      dbuff[i + 1] = y;
      i += 2;
...
#ifdef PRINT_INFO
  mexPrintf("DGNLLDUtsol: multiplication by block diagonal matrix done\n");
  fflush(stdout);
#endif

  /* back substitution with transposed unit lower triangular matrix */
  for (i = n - 1; i >= 0; i--) {
...
#ifdef PRINT_INFO
  mexPrintf("DGNLLDUtsol: transposed backward substitution done\n");
  fflush(stdout);
#endif
  ...
  for (i = 0; i < n; i++) {
    /* the input vector counts permutations from 1,...,n */
    j = pPvecr[i] - 1;
    pzr[j] = pSlvecr[i] * dbuff[i];
  } /* end for i */
#ifdef PRINT_INFO
  mexPrintf(
      "DGNLLDUtsol: permutation + transposed right diagonal scaling done\n");
  fflush(stdout);
#endif
