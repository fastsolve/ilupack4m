...
)
{
...
#ifdef PRINT_INFO
    mexPrintf("DGNLLDUtsol: permutation + transposed left diagonal scaling done\n");fflush(stdout);
#endif
    ...
    /* multiplication with a transposed block diagonal matrix with 1x1 and 2x2 pivots */
    i=0;
    j=iD_ia[0];
    while (i<n) {
          k=iD_ia[i+1];
...
#ifdef PRINT_INFO
    mexPrintf("DGNLLDUtsol: permutation + transposed right diagonal scaling done\n");fflush(stdout);
#endif
