...
)
{
    ...
    integer         i,j,k,l,n;
    doubleprecision  *pPvecr, *pSvecr, *pzr, *pyr, *dbuffr, xr, yr, 
                     *pPveci, *pSveci, *pzi, *pyi, *dbuffi, xi, yi;
    size_t          mrows, ncols;
       ...
       while (i<n) {
		...
		if (l-k>1) { /* 2x2 pivot with four nonzero elements */
		   ...
		   yr=iD_valuesR[j+1]*dbuffr[i]  -iD_valuesI[j+1]*dbuffi[i]
		     +iD_valuesR[k+1]*dbuffr[i+1]-iD_valuesI[k+1]*dbuffi[i+1];
		   ...
		   yi=iD_valuesR[j+1]*dbuffi[i]  +iD_valuesI[j+1]*dbuffr[i]
		     +iD_valuesR[k+1]*dbuffi[i+1]+iD_valuesI[k+1]*dbuffr[i+1];
		}
    ...
    if (L_valuesI!=NULL) {
	   ...
	   for (k=L_ia[i]+1; k<L_ia[i+1]; k++) {
	       /* row index */
	       j=L_ja[k];
	       dbuffr[i]-=L_valuesR[k]*dbuffr[j]+L_valuesI[k]*dbuffi[j]; /* conjugate L! */
	       dbuffi[i]-=L_valuesR[k]*dbuffi[j]-L_valuesI[k]*dbuffr[j]; /* conjugate L! */
	   } /* end for k */
       } /* end for i */
    }
    else {
