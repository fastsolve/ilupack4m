/* $Id: ZHERLDLsol.c 799 2015-08-12 14:52:16Z bolle $ */
/* ========================================================================== */
/* === ZHERLDLsol mexFunction =========================================== */
/* ========================================================================== */
...
#ifdef PRINT_INFO
  mexPrintf("ZHERLDLsol: input parameter y imported\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("ZHERLDLsol: input parameter Pvec imported\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("ZHERLDLsol: input parameter Svec imported\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("ZHERLDLsol: input parameter L imported\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("ZHERLDLsol: input parameter iD imported\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("ZHERLDLsol: output vector and buffer allocated\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("ZHERLDLsol: permutation + diagonal scaling done\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("ZHERLDLsol: forward substitution done\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("ZHERLDLsol: multiplication by block diagonal matrix done\n");
  fflush(stdout);
#endif

  /* back substitution with unit upper triangular matrix U=L^H, conjugate!!! */
  if (L_valuesI != NULL) {
    for (i = n - 1; i >= 0; i--) {
      ...
      for (k = L_ia[i] + 1; k < L_ia[i + 1]; k++) {
        ...
        dbuffr[i] -= L_valuesR[k] * dbuffr[j] +
                     L_valuesI[k] * dbuffi[j]; /* conjugate L! */
        dbuffi[i] -= L_valuesR[k] * dbuffi[j] -
                     L_valuesI[k] * dbuffr[j]; /* conjugate L! */
      }                                        /* end for k */
    }                                          /* end for i */
  } else {
    for (i = n - 1; i >= 0; i--) {
...
#ifdef PRINT_INFO
  mexPrintf("ZHERLDLsol: backward substitution done\n");
  fflush(stdout);
#endif
