...
void mexFunction(
  ...
  integer i, j, k, l, m, n, p, q, r, s, t, ii, jj, flag, *ipiv, size_gemm_buff,
      ...
      mi_size, i_first, j_first, k_first, Ji_cont, Ik_cont, Ii_cont;
  double val, alpha, beta, *Dbuff, *work, *gemm_buff, *pr, *pr2, *pr3, *pr4,
      *prBLJ, *prBLI, *prBLL, *prBLD, *prBDD, *prBLinvJ, *prBLinvI, *prBLinvL,
      *prBLinvD, *prBLinvJi, *prBLinvIi, *prBLinvLi, *prBLinvDi;
  mwIndex *ja, *ia;
      ...
      if (Ii_cont && Ik_cont && Ji_cont) {
        ...
        if (t)
          dgemm_("N", "N", &t, &n_size, &ii, &alpha,
                 prBLinvLi + i_first + mi_size * j_first, &mi_size, prBLL + l,
                 &m_size, &beta, prBLinvL + k_first, &m_size, 1, 1);
#ifdef PRINT_INFO
        mexPrintf("Ik=[");
        r = 0;
        s = 0;
        ...
        mexPrintf("];\n");
        mexPrintf("DSYMselbinv: BLinv{%d}.L(Ik,:) = - BLinv{%d}.L(Ii,Ji)  "
        ...
        if (t)
          dgemm_("T", "N", &ii, &n_size, &t, &alpha,
                 ...
                 prBLL + k_first, &m_size, &beta, prBLinvL + l, &m_size, 1, 1);
#ifdef PRINT_INFO
        mexPrintf("Ik=[");
        r = 0;
        s = 0;
        ...
        mexPrintf("];\n");
        mexPrintf("DSYMselbinv: BLinv{%d}.L(%d:%d,:) = - BLinv{%d}.L(Ii,Ji)' "
         ...
         * BLinv{k}.L(l:j,:)  */
        dgemm_("N", "N", &ii, &n_size, &ii, &alpha,
               prBLinvDi + j_first + j_first * ni_size, &ni_size, prBLL + l,
               &m_size, &beta, prBLinvL + l, &m_size, 1, 1);
#ifdef PRINT_INFO
        r = l;
        s = 0;
        mexPrintf("Ji=[");
        ...
        mexPrintf("];\n");
        mexPrintf("DSYMselbinv: BLinv{%d}.L(%d:%d,:) = - BLinv{%d}.D(Ji,Ji)  "
...
/* determine amount of auxiliary memory */
        ...
        if (level3_BLAS && t) {
...
#ifdef PRINT_INFO
            mexPrintf(
                "DSYMselbinv: cached zeros instead of  BLinv{%d}.L(Ik,:)\n",
                k + 1);
            fflush(stdout);
#endif
          } else {
          ...
          /* call level 3 BLAS */
          ii = j - l + 1;
          dgemm_("N", "N", &t, &n_size, &ii, &alpha, pr, &p, prBLL + l, &m_size,
                 &beta, pr2, &q, 1, 1);
#ifdef PRINT_INFO
          mexPrintf("Ik=[");
          r = 0;
          ...
          if (Ik_cont)
            ...
            mexPrintf("DSYMselbinv: cached                BLinv{%d}.L(Ii,Ji)  "
                      "*BL{%d}.L(%d:%d,:)\n",
                      i + 1, k + 1, l + 1, j + 1);
...
/* BLinv{k}.L(Ik,:)  -=  BLinv{i}.L(Ii,Ji) * BL{k}.L(l:j,:) */
        ...
        if (level3_BLAS && t) {
...
#ifdef PRINT_INFO
            mexPrintf(
                "DSYMselbinv: cached copy of BL{%d}.L(Ik,:)\nIndex set J:\n",
                i + 1);
            fflush(stdout);
            for (q = 0; q < n_size; q++)
          ...
          mexPrintf("];\n");
          mexPrintf("DSYMselbinv: BLinv{%d}.L(%d:%d,:) = - "
...
/* BLinv{k}.L(l:j,:) -=  BLinv{i}.L(Ii,Ji)^T * BL{k}.L(Ik,:) */
        ...
        if (!Ji_cont) {
...
#ifdef PRINT_INFO
          mexPrintf(
              "DSYMselbinv: cached copy of BLinv{%d}.D(Ji,Ji)\nIndices:\n",
              i + 1);
          fflush(stdout);
          p = l;
          q = 0;
          ...
          pr = prBLinvDi + j_first + j_first * ni_size;
          p = ni_size;
        } /* end if-else */
        ...
        beta = 1.0;
        ii = j - l + 1;
        dgemm_("N", "N", &ii, &n_size, &ii, &alpha, pr, &p, prBLL + l, &m_size,
               &beta, prBLinvL + l, &m_size, 1, 1);
#ifdef PRINT_INFO
        mexPrintf("Ik=[");
        r = 0;
        s = 0;
    ...
    ii = j - l + 1;
    if (m_size)
      dgemm_("T", "N", &n_size, &n_size, &m_size, &alpha, prBLL, &m_size,
             prBLinvL, &m_size, &beta, prBLinvD, &n_size, 1, 1);
