...
void mexFunction(
  ...
  */
  ...
  PRE =
      (DAMGlevelmat *)MAlloc((size_t)sizeof(DAMGlevelmat), "DSPDilupackfactor");
  ...
  if (nlhs == 5 && ierr == 0) {
    ...
    *pr = param->rcomflag;
    ...
    if (param->rcomflag != 0) {
      ...
      plhs[3] =
          mxCreateSparse((mwSize)param->A.nr, (mwSize)param->A.nc, nnz, mxREAL);
      S_output = plhs[3];
  ...
  if (param->nthreads == 1) {
    plhs[0] =
        mxCreateStructMatrix((mwSize)1, (mwSize)PRE->nlev, SIZE_PNAMES, pnames);
    if (plhs[0] == NULL)
      mexErrMsgTxt("Could not create structure mxArray\n");
    PRE_output = plhs[0];
      ...
      if (param->rcomflag == 0) {
        ...
        if (jstruct == PRE->nlev - 1 &&
          ...
          for (i = 0; i < ((PRE->issingle) ? scurrent->nB : current->nB); i++) {
            ...
            l = ((PRE->issingle) ? scurrent->LU.ia[i] : current->LU.ia[i]) - 1;
            dbuf =
                1.0 / ((PRE->issingle) ? (double)spr[ibuff[l]] : pr[ibuff[l]]);
#else
            l = i;
      ...
      if (jstruct >= PRE->nlev - 1) {
        ...
        */
        ...
        if (PRE->issingle)
          ...
          fout = mxCreateSparse((mwSize)0, (mwSize)0, (mwSize)0, mxREAL);
        /*
          mexPrintf("level %d, coarse grid system A_H
          discarded\n",jstruct+1);fflush(stdout);
        */
      } else {
        /* switched to full-matrix processing? */
        if (jstruct == PRE->nlev - 2 &&
          ...
          /*
            mexPrintf("level %d, switch to full-matrix processing on the next
            level\n",jstruct+1);fflush(stdout);
          */
...
/* additional permutation? */
            ...
            */
            ...
            if (param->flags & COARSE_REDUCE) {
              if (PRE->issingle)
                fout = mxCreateSparse((mwSize)scurrent->nB,
                                      (mwSize)scurrent->nB, nnz, mxREAL);
              else
                fout = mxCreateSparse((mwSize)current->nB, (mwSize)current->nB,
                                      nnz, mxREAL);
          ...
          if (jstruct == nlev - 1 &&
            ...
            for (i = 0; i < ((PRE->issingle) ? scurrent->nB : current->nB);
              ...
              l = ((PRE->issingle) ? scurrent->LU.ia[i] : current->LU.ia[i]) -
                  1;
              dbuf = 1.0 /
                     ((PRE->issingle) ? (double)spr[ibuff[l]] : pr[ibuff[l]]);
#else
              l = i;
              dbuf = ((PRE->issingle) ? (double)spr[ibuff[l]] : pr[ibuff[l]]);
#endif
...
#ifndef USE_LAPACK_DRIVER
              sr[i] = 1.0 /
                      ((PRE->issingle) ? scurrent->LU.a[i] : current->LU.a[i]);
#else
              sr[i] = ((PRE->issingle) ? scurrent->LU.a[i] : current->LU.a[i]);
#endif
            }
            ...
            */
            ...
            if (PRE->issingle) {
              ...
              mexPrintf("%2d. field %s{%d}(%d).%s...else...else...issingle\n",
                        ifield + 1, pnames_omproot[ifield], iomp + 1,
                        jstruct + 1, ppnames[jfield]);
              fflush(stdout);
#endif
              for (i = 0; i < scurrent->next->A.nr; i++) {
                jcs[i] = k;
                ...
                j = scurrent->next->A.ia[i] - 1;
                jj = scurrent->next->A.ia[i + 1] - scurrent->next->A.ia[i];
                Sqsort(scurrent->next->A.a + j, scurrent->next->A.ja + j,
                       istack, &jj);
        ...
        if (j) {
          ...
          if (PRE->issingle)
            fout = mxCreateSparse((mwSize)scurrent->A.nr,
                                  (mwSize)scurrent->A.nc, nnz, mxREAL);
          else
