...
void mexFunction(
  ...
  mwIndex *irs, *jcs, *A_ja, /* row indices of input matrix A */
  ...
  A.ia = (integer *)MAlloc((size_t)(A.nc + 1) * sizeof(integer),
                           "ZGNLDSYMilupacksolver");
  A.ja =
      (integer *)MAlloc((size_t)nnz * sizeof(integer), "ZGNLDSYMilupacksolver");
  ...
  */
  ...
  classIDflags =
      (mxClassID *)mxCalloc((size_t)nfields + 1, (size_t)sizeof(mxClassID));
    ...
    if (classIDflags[ifield] == mxCHAR_CLASS) {
      ...
      if (!strcmp("amg", fnames[ifield])) {
        if (strcmp(param->amg, input_buf)) {
          param->amg =
              (char *)MAlloc((size_t)buflen * sizeof(char), "ilupacksolver");
        ...
        if (strcmp(param->postsmoother, input_buf)) {
          param->postsmoother =
              (char *)MAlloc(buflen * sizeof(char), "ilupacksolver");
  ...
  /* get size of input matrix A */
  rhs = (doublecomplex *)MAlloc((size_t)A.nr * sizeof(doublecomplex),
                                "ZGNLDSYMilupacksolver:rhs");
  pr = mxGetPr(b_input);
  ...
  /* numerical solution */
  sol = (doublecomplex *)MAlloc((size_t)A.nr * sizeof(doublecomplex),
                                "ZGNLDSYMilupacksolver:sol");
  pr = mxGetPr(x0_input);
  ...
  if (j == -1)
    plhs[1] =
        mxCreateStructMatrix((mwSize)1, (mwSize)1, (mwSize)nfields + 1, fnames);
  else
    plhs[1] =
        mxCreateStructMatrix((mwSize)1, (mwSize)1, (mwSize)nfields, fnames);
  if (plhs[1] == NULL)
  ...
  *pr = param->ipar[26];
  ...
  case -1: /* too many iteration steps */
    ...
    mexPrintf("number of iteration steps exceeded its limit.\nEither increase "
              "`options.maxit'\n or recompute ILUPACK preconditioner using a "
              "smaller `options.droptol'");
    break;
  ...
  case -3: /* breakdown */
    mexErrMsgTxt("iterative solver breaks down.\nMost likely you need to "
                 "recompute ILUPACK preconditioner using a smaller "
                 "`options.droptol'");
    plhs[0] = NULL;
    break;
