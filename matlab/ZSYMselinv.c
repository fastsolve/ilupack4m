/* ========================================================================== */
/* === ZSYMselinv mexFunction =============================================== */
/* ========================================================================== */

/*
    Usage:

    compute approximate selective matrix inverse for a given LDL^T factorization
    
    Example:

    % for initializing parameters
    Linv=ZSYMselinv(L,D,p, Delta)
...
)
{
    ...
    doubleprecision *ainvr, *ainvi, Djm1jm1r,Djm1jm1i, Djjm1r,Djjm1i, Djjr,Djji, 
                    detr,deti, nrm;
    size_t          mrows, ncols;
...
#ifdef PRINT_INFO
    mexPrintf("ZSYMselinv: input parameter L imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZSYMselinv: input parameter D imported\n");fflush(stdout);
#endif
    ...
    p    =(integer *)MAlloc((size_t)n*sizeof(integer),"ZSYMselinv:p");
    invp =(integer *)MAlloc((size_t)n*sizeof(integer),"ZSYMselinv:invp");
    ibuff=(integer *)MAlloc((size_t)n*sizeof(integer),"ZSYMselinv:ibuff");
    for (i=0; i<n; i++) {
        j=p_valuesR[i]-1;
        if (j<0 || n<=j) {
...
#ifdef PRINT_INFO
    mexPrintf("ZSYMselinv: input parameter p imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZSYMselinv: input parameter Delta imported\n");fflush(stdout);
#endif

#ifdef PRINT_INFO
    mexPrintf("ZSYMselinv: input parameters imported\n");fflush(stdout);
#endif


    /* set up preliminary structure for every column of ainv */
    /* create memory for output matrix Ainv, here: ia */
    ia   =(integer *)MAlloc((size_t)(n+1)  *sizeof(integer),"ZSYMselinv:ia");
    i=0;
    ia[0]=0;
...
#ifdef PRINT_INFO
    mexPrintf("ZSYMselinv: additional memory for blocked columns: %d\n",nnz);fflush(stdout);
#endif
    nnz+=ia[n];

    ja   =(integer *)MAlloc((size_t)nnz*sizeof(integer),"ZSYMselinv:ja");
    ainvr=(double *) MAlloc((size_t)nnz*sizeof(double), "ZSYMselinv:ainvr");
    ainvi=(double *) MAlloc((size_t)nnz*sizeof(double), "ZSYMselinv:ainvi");
    for (i=0; i<nnz; i++) {
        ainvr[i]=ainvi[i]=0.0;
    } /* end for i */
...
#ifdef PRINT_INFO
    mexPrintf("ZSYMselinv: row index structure inserted\n");fflush(stdout);
    for (i=0; i<n; i++) {
...
#endif
       mm=ia[j];
       nrm=(flag_piD)? 1.0/(D_valuesR[m]*D_valuesR[m]+D_valuesI[m]*D_valuesI[m]) : 1.0/(D_valuesR[m]*D_valuesR[m]);
	  
       ainvr[mm]= D_valuesR[m]*nrm;
       ainvi[mm]=(flag_piD)? -D_valuesI[m]*nrm : 0.0;
...
#endif
       Djm1jm1r=D_valuesR[m];
       Djm1jm1i=(flag_piD)? D_valuesI[m] : 0.0;
...
#endif
       Djjr   =D_valuesR[m];
       Djji   =(flag_piD)? D_valuesI[m] : 0.0;
       /* determinant for 2x2 matrix inverse */
       detr=Djm1jm1r*Djjr-Djm1jm1i*Djji-(Djjm1r*Djjm1r-Djjm1i*Djjm1i);
       deti=Djm1jm1r*Djji+Djm1jm1i*Djjr-(Djjm1r*Djjm1i+Djjm1i*Djjm1r);
       nrm=1.0/(detr*detr+deti*deti);
...
#endif
       ainvr[mm]=( Djjr*detr+Djji*deti)*nrm;
       ainvi[mm]=(-Djjr*deti+Djji*detr)*nrm;
       ainvr[mm+1]=-( Djjm1r*detr+Djjm1i*deti)*nrm;
       ainvi[mm+1]=-(-Djjm1r*deti+Djjm1i*detr)*nrm;
       mm=ia[j];
#ifdef PRINT_CHECK
       if (ja[mm]!=j) { mexPrintf("ainv: mismatch (%d,%d)\n",ja[mm]+1,j+1);fflush(stdout);}
#endif
       ainvr[mm]=( Djm1jm1r*detr+Djm1jm1i*deti)*nrm;
       ainvi[mm]=(-Djm1jm1r*deti+Djm1jm1i*detr)*nrm;
    ...
    while (j>=0) {
	  ...
	  if (!flag) {
	     ...
	     /* Compute Ainv(j-1:j,j-1:j)=D(j-1:j,j-1:j)^{-1}-L(I,j-1:j)^T*Ainv(I,j-1:j) */
	     /* extract D(j-1:j,j-1:j) */
	     m=D_ia[j-1];
#ifdef PRINT_CHECK
	     if (D_ja[m]!=j-1) { mexPrintf("D: diagonal mismatch (%d,%d)\n",D_ja[m]+1,j);fflush(stdout);}
       ...
       if (D_ja[D_ia[j]]!=j-1) { mexPrintf("D: super-diagonal mismatch (%d,%d)\n",D_ja[D_ia[j]]+1,j);fflush(stdout);}
	     ...
	     /* END Computation Ainv(j-1:j,j-1:j)=D(j,j)^{-1}-L(I,j-1:j)^T*Ainv(I,j-1:j) */

	     j=j-2;
	  } /* end if-else */
    } /* end while i */
#ifdef PRINT_INFO
    mexPrintf("ZSYMselinv: numerical values structure inserted\n");fflush(stdout);
    for (i=0; i<n; i++) {
        mexPrintf("row indices column %d\n", i+1);
        for (j=ia[i]; j<ia[i+1]; j++) {
	    mexPrintf("%8ld", ja[j]+1);
...
#ifdef PRINT_INFO
    mexPrintf("ZSYMselinv: exported matrix reordered\n");fflush(stdout);
    for (i=0; i<n; i++) {
        mexPrintf("row indices column %d\n", i+1);
...
#ifdef PRINT_INFO
    mexPrintf("ZSYMselinv: exported matrix rescaled and indices sorted\n");fflush(stdout);
    for (i=0; i<n; i++) {
        mexPrintf("row indices column %d\n", i+1);
        for (j=Ainv_ia[i]; j<Ainv_ia[i+1]; j++) {
	    mexPrintf("%8ld", Ainv_ja[j]+1);
...
#ifdef PRINT_INFO
    mexPrintf("ZSYMselinv: memory released\n");fflush(stdout);
#endif
