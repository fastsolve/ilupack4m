...
)
{
    ...
    integer         i,j,k,l,m,kk,ll,mm,n,flagkm1,flag,flagkp1,nnz,
                    *p, *invp, ierr, *ibuff, flag_piL, flag_piD, flag_piUT;
    doublecomplex   Djm1jm1, Djjm1, Djm1j, Djj, det, *Linv_a, *Dinv_a,*UTinv_a;
    size_t          mrows, ncols;
    double          *L_valuesR, *D_valuesR, *UT_valuesR, *Ainv_valuesR,
                    *L_valuesI, *D_valuesI, *UT_valuesI, *Ainv_valuesI,
                    *p_valuesR, *Deltal_valuesR, *Deltar_valuesR, nrm;
    ...
    Linv_ja=(integer *)MAlloc((size_t)nnz*sizeof(integer),"ZGNLselinv:Linv_ja");
    Linv_a =(doublecomplex *) MAlloc((size_t)nnz*sizeof(doublecomplex), "ZGNLselinv:Linv_a");
    for (i=0; i<nnz; i++) {
    ...
    UTinv_ja=(integer *)MAlloc((size_t)nnz*sizeof(integer),"ZGNLselinv:UTinv_ja");
    UTinv_a =(doublecomplex *) MAlloc((size_t)nnz*sizeof(doublecomplex), "ZGNLselinv:UTinv_a");
    for (i=0; i<nnz; i++) {
    ...
    Dinv_ia   =(integer *)MAlloc((size_t)(n+1)  *sizeof(integer),"ZGNLselinv:Dinv_ia");
    i=0;
    Dinv_ia[0]=0;
    /* count additional fill-in for block columns of size 2 */
    nnz=0;
    ...
    Dinv_ja=(integer *)MAlloc((size_t)nnz*sizeof(integer),"ZGNLselinv:Dinv_ja");
    Dinv_a =(doublecomplex *) MAlloc((size_t)nnz*sizeof(doublecomplex), "ZGNLselinv:Dinv_a");
    for (i=0; i<nnz; i++) {
    ...
    if (!flag) {
       ...
       det.r=Djm1jm1.r*Djj.r-Djm1jm1.i*Djj.i-(Djjm1.r*Djm1j.r-Djjm1.i*Djm1j.i);
       det.i=Djm1jm1.r*Djj.i+Djm1jm1.i*Djj.r-(Djjm1.r*Djm1j.i+Djjm1.i*Djm1j.r);
	     ...
	     */
		 ...
		 while (l<UTinv_ia[j+1] && mm<UTinv_ia[k+1]) {
			  ...
			  UTinv_a[l].r-=(flag_piL)?
			                UTinv_a[mm].r*L_valuesR[m]-UTinv_a[mm].i*L_valuesI[m]:
			                UTinv_a[mm].r*L_valuesR[m];
			  UTinv_a[l].i-=(flag_piL)?
			                UTinv_a[mm].r*L_valuesI[m]+UTinv_a[mm].i*L_valuesR[m]:
		 ...
		 /* block diagonal case Dinv(:,k) */
		    ...
		    if (Dinv_ja[kk]==k-1 && flagkm1>=0) {
		       ...
		       UTinv_a[flagkm1].r-=(flag_piL)?
			                   Dinv_a[kk].r*L_valuesR[m]-Dinv_a[kk].i*L_valuesI[m]:
			                   Dinv_a[kk].r*L_valuesR[m];
		       UTinv_a[flagkm1].i-=(flag_piL)?
			                   Dinv_a[kk].r*L_valuesI[m]+Dinv_a[kk].i*L_valuesR[m]:
			                                             Dinv_a[kk].i*L_valuesR[m];
		    }
		    ...
		    if (Dinv_ja[kk]==k && flag>=0) {
		       ...
		       /* UTinv(k+1,j)=UTinv(k+1,j)-Dinv(k+1,k)*L(k,j) */
		       UTinv_a[flagkp1].r-=(flag_piL)?
			                   Dinv_a[kk].r*L_valuesR[m]-Dinv_a[kk].i*L_valuesI[m]:
			                   Dinv_a[kk].r*L_valuesR[m];
		       UTinv_a[flagkp1].i-=(flag_piL)?
			                   Dinv_a[kk].r*L_valuesI[m]+Dinv_a[kk].i*L_valuesR[m]:
			                                             Dinv_a[kk].i*L_valuesR[m];
		    }
		 }
		 ...
		 while (l<Linv_ia[j+1] && mm<Linv_ia[k+1]) {
			  ...
			  Linv_a[l].r-=(flag_piUT)?
			               Linv_a[mm].r*UT_valuesR[m]-Linv_a[mm].i*UT_valuesI[m]:
			               Linv_a[mm].r*UT_valuesR[m];
			  Linv_a[l].i-=(flag_piUT)?
			               Linv_a[mm].r*UT_valuesI[m]+Linv_a[mm].i*UT_valuesR[m]:
		 ...
		 while (l<UT_ia[j+1] && mm<UTinv_ia[k+1]) {
			  ...
			  mm++;
		          ...
		          Linv_a[m].r-=(flag_piUT)?
			               UTinv_a[mm].r*UT_valuesR[l]-UTinv_a[mm].i*UT_valuesI[l]:
			               UTinv_a[mm].r*UT_valuesR[l];
		          Linv_a[m].i-=(flag_piUT)?
			               UTinv_a[mm].r*UT_valuesI[l]+UTinv_a[mm].i*UT_valuesR[l]:
