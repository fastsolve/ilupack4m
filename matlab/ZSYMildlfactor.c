...
)
{
    ...
    pcolscale=(doublecomplex *)MAlloc((size_t)A.nc*sizeof(doublecomplex),"ZSYMildlfactor");
    prowscale=pcolscale;
    nB=A.nc;
    if (options.matching) {
       if (!strcmp("metise",options.ordering)) {
#ifdef _MC64_MATCHING_    
	  ierr=ZSYMperm_mc64_metis_e(A, prowscale, pcolscale, p, invq, &nB, &options);
#else
	  ierr=ZSYMperm_matching_metis_e(A, prowscale, pcolscale, p, invq, &nB, &options);
#endif
#ifdef PRINT_INFO
...
#else
	  ierr=ZSYMperm_matching_amd(A, prowscale, pcolscale, p, invq, &nB, &options);
#endif
#ifdef PRINT_INFO
...
#else
	  ierr=ZSYMperm_matching_rcm(A, prowscale, pcolscale, p, invq, &nB, &options);
#endif
#ifdef PRINT_INFO
...
#ifdef _MC64_MATCHING_    
	  ierr=ZSYMperm_mc64_metis_n(A, prowscale, pcolscale, p, invq, &nB, &options);
#else
	  ierr=ZSYMperm_matching_metis_n(A, prowscale, pcolscale, p, invq, &nB, &options);
#endif
#ifdef PRINT_INFO
    ...
    /* auxiliary buffers */
    ibuff=(integer *)      MAlloc((size_t)14*A.nc*sizeof(integer),      "ZSYMildlfactor");
    dbuff=(doublecomplex *)MAlloc((size_t) 4*A.nc*sizeof(doublecomplex),"ZSYMildlfactor");

    /* ilu buffers */
    /* just a simple guess at least 2n+1! */
    mem=ELBOW*A.ia[A.nr]+1;
    jlu=(integer *)      MAlloc((size_t)mem*sizeof(integer),      "ZSYMildlfactor");
    alu=(doublecomplex *)MAlloc((size_t)mem*sizeof(doublecomplex),"ZSYMildlfactor");
...
#endif
	  jlu=(integer *)      ReAlloc(jlu,mem*sizeof(integer),      "ZSYMildlfactor:jlu");
	  alu=(doublecomplex *)ReAlloc(alu,mem*sizeof(doublecomplex),"ZSYMildlfactor:alu");
#ifdef PRINT_INFO
    ...
    while (i<A.nc) {
	     ...
	     A_valuesI[k]=-alu[i].i/dbuf;
             ...
             dbufi=  alu[i].r*alu[i+1].i+alu[i].i*alu[i+1].r
	           -(alu[A.nc+1+i].r*alu[A.nc+1+i].i+alu[A.nc+1+i].i*alu[A.nc+1+i].r);
	     ...
	     A_valuesR[k+1]=-( alu[A.nc+1+i].r*dbufr+alu[A.nc+1+i].i*dbufi)/dbuf;
	     A_valuesI[k+1]=-(-alu[A.nc+1+i].r*dbufi+alu[A.nc+1+i].i*dbufr)/dbuf;
	     ...
	     k+=2;
             ...
             A_ja[k+1]=i+1;
	     /* values */
	     A_valuesR[k]  =-( alu[A.nc+1+i].r*dbufr+alu[A.nc+1+i].i*dbufi)/dbuf;
	     A_valuesI[k]  =-(-alu[A.nc+1+i].r*dbufi+alu[A.nc+1+i].i*dbufr)/dbuf;
