...
void mexFunction(
  ...
  doublecomplex *work, *gemm_buff, *pz, *pz2, *pz3, *pz4, alpha, beta, *BLLbuff,
      ...
      *pzBLinvDi, *pzBLinvLi;
  double val, *Dbuffr, *pr, *pr2, *prBLJ, *prBLI, *prBLL, *piBLL, *prBLD,
      *piBLD, *prBDD, *piBDD, *prBLinvJ, *prBLinvI, *prBLinvL, *piBLinvL,
      *prBLinvD, *piBLinvD, *prBLinvJi, *prBLinvIi;
  mwIndex *ja, *ia;
  ...
  gemm_buff = (doublecomplex *)MAlloc(
      (size_t)size_gemm_buff * sizeof(doublecomplex), "ZHERselbinv:gemm_buff");
...
/* remember that the structure stores indices from 1,...,n */
  ...
  */
  BLinvDbuff = (doublecomplex **)MAlloc(
      (size_t)nblocks * sizeof(doublecomplex *), "ZHERselbinv:BLinvDbuff");
  BLinvLbuff = (doublecomplex **)MAlloc(
      (size_t)nblocks * sizeof(doublecomplex *), "ZHERselbinv:BLinvLbuff");
  n_BLLbuff = 0;
  for (i = 0; i < nblocks; i++) {
    /* extract BL{i} */
    BL_block = mxGetCell(BL, i);
    ...
    BLinvDbuff[i] = (doublecomplex *)MAlloc(
        (size_t)k * l * sizeof(doublecomplex), "ZHERselbinv:BLinvDbuff[i]");
    ...
    BLinvLbuff[i] = (doublecomplex *)MAlloc(
        (size_t)k * l * sizeof(doublecomplex), "ZHERselbinv:BLinvLbuff[i]");
...
*/
      ...
      */
      if (Ii_cont && Ik_cont && Ji_cont) {
        ...
        mexPrintf("];\n");
        mexPrintf("ZHERselbinv: BLinv{%d}.L(Ik,:) = - BLinv{%d}.L(Ii,Ji)  "
        ...
        mexPrintf("];\n");
        mexPrintf("ZHERselbinv: BLinv{%d}.L(%d:%d,:) = - BLinv{%d}.L(Ii,Ji)' "
 ...
 */
        ...
        mexPrintf("];\n");
        mexPrintf("ZHERselbinv: BLinv{%d}.L(%d:%d,:) = - BLinv{%d}.D(Ji,Ji)  "
...
/* determine amount of auxiliary memory */
        ...
        if (level3_BLAS && t) {
...
#ifdef PRINT_INFO
            mexPrintf(
                "ZHERselbinv: cached zeros instead of  BLinv{%d}.L(Ik,:)\n",
                k + 1);
            fflush(stdout);
#endif
          } else {
          ...
          if (Ik_cont)
            ...
            mexPrintf("ZHERselbinv: cached                BLinv{%d}.L(Ii,Ji)  "
                      "*BL{%d}.L(%d:%d,:)\n",
                      i + 1, k + 1, l + 1, j + 1);
...
/* BLinv{k}.L(Ik,:)  -=  BLinv{i}.L(Ii,Ji) * BL{k}.L(l:j,:) */
        ...
        if (level3_BLAS && t) {
...
#ifdef PRINT_INFO
            mexPrintf(
                "ZHERselbinv: cached copy of BL{%d}.L(Ik,:)\nIndex set J:\n",
                i + 1);
            fflush(stdout);
            for (q = 0; q < n_size; q++)
          ...
          if (ii && n_size && t) {
            ...
            mexPrintf("call zgemm_\n");
#endif
            zgemm_("C", "N", &ii, &n_size, &t, &alpha, pz, &p, pz2, &q, &beta,
          ...
          mexPrintf("];\n");
          mexPrintf("ZHERselbinv: BLinv{%d}.L(%d:%d,:) = - "
                    "BLinv{%d}.L(Ii,Ji)^H*BL{%d}.L(Ik,:)  + "
                    "BLinv{%d}.L(%d:%d,:)\n",
                    k + 1, l + 1, j + 1, i + 1, k + 1, k + 1, l + 1, j + 1);
          for (jj = l; jj <= j; jj++) {
            for (q = 0; q < n_size; q++)
                ...
                while (s < ni_size) {
                  ...
                  */
                  if ((integer)prBLinvJi[s] == (integer)prBLI[r]) {
                     ...
                     */
                    pz3->r -=
                        (pz2->r) * (pz->r) + (pz2->i) * (pz->i); /* conjug! */
                    pz3->i -=
                        (pz2->r) * (pz->i) - (pz2->i) * (pz->r); /* conjug! */
                    pz3++;
                    r++;
                  }
                  s++;
...
#ifdef PRINT_INFO
          mexPrintf(
              "ZHERselbinv: cached copy of BLinv{%d}.D(Ji,Ji)\nIndices:\n",
              i + 1);
          fflush(stdout);
          p = l;
          q = 0;
...
/* use LAPACK's zhetri_ for matrix inversion given the LDL^H decompositon */
    ...
    if (n_size && m_size) {
      ...
      mexPrintf("call zgemm_\n");
#endif
      zgemm_("C", "N", &n_size, &n_size, &m_size, &alpha, pzBLL, &m_size,
