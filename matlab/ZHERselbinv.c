/* $Id: ZHERselbinv.c 791 2015-08-05 08:51:07Z bolle $ */
/* ========================================================================== */
/* === ZHERselbinv mexFunction ============================================== */
/* ========================================================================== */
...
/*
    ...
    block LDL^H factorization

    Example:

    % for initializing parameters
    [D, BLinv]=ZHERselbinv(BL,BD,perm, Delta)
...
)
{
    ...
    integer      i,j,k,l,m,n,p,q,r,s,t,
                 ...
                 i_first, j_first, k_first, n_BLLbuff,
                 Ji_cont, Ik_cont, Ii_cont;
    doublecomplex *work, *gemm_buff, *pz, *pz2, *pz3, *pz4, alpha, beta,
                  *BLLbuff, **BLinvDbuff, **BLinvLbuff,
                  *pzBLD, *pzBLL, *pzBLinvD, *pzBLinvL, *pzBLinvDi, *pzBLinvLi;
    double       val,  *Dbuffr,
                 *pr, *pr2,
                 *prBLJ,     *prBLI,     *prBLL,     *piBLL,     *prBLD,     *piBLD,
                                                                 *prBDD,     *piBDD,
                 *prBLinvJ,  *prBLinvI,  *prBLinvL,  *piBLinvL,  *prBLinvD,  *piBLinvD,
                 *prBLinvJi, *prBLinvIi;
    mwIndex      *ja, *ia;
...
#ifdef PRINT_INFO
    mexPrintf("ZHERselbinv: input parameter BL imported\n");fflush(stdout);
#endif
...
#endif
#ifdef PRINT_INFO
    mexPrintf("ZHERselbinv: input parameter BD imported\n");fflush(stdout);
#endif
...
#endif
#ifdef PRINT_INFO
    mexPrintf("ZHERselbinv: input parameter perm imported\n");fflush(stdout);
#endif
...
#endif
#ifdef PRINT_INFO
    mexPrintf("ZHERselbinv: input parameter Delta imported\n");fflush(stdout);
#endif

#ifdef PRINT_INFO
    mexPrintf("ZHERselbinv: input parameters imported\n");fflush(stdout);
#endif
    ...
    /* auxiliary arrays for inverting diagonal blocks using zhetri_ */
    ipiv =(integer *)     MAlloc((size_t)n*sizeof(integer),      "ZHERselbinv:ipiv");
    work=(doublecomplex *)MAlloc((size_t)n*sizeof(doublecomplex),"ZHERselbinv:work");
    /* auxiliary buff for output D */
    Dbuffr=(doubleprecision *)MAlloc((size_t)n*sizeof(doubleprecision),"ZHERselbinv:Dbuffr");
    /* auxiliary buffer for level 3 BLAS */
    size_gemm_buff=n;
    gemm_buff=(doublecomplex *)MAlloc((size_t)size_gemm_buff*sizeof(doublecomplex),"ZHERselbinv:gemm_buff");

    /* inverse mapping index -> block number */
    block=(integer *)CAlloc((size_t)n,sizeof(integer),"ZHERselbinv:block");
    for (i=0; i<nblocks; i++) {
...
#ifdef PRINT_INFO
    mexPrintf("ZHERselbinv: inverse mapping index -> block number computed\n");fflush(stdout);
    for (i=0; i<n; i++)
    ...
    */
    BLinvDbuff=(doublecomplex **)MAlloc((size_t)nblocks*sizeof(doublecomplex *),
					 "ZHERselbinv:BLinvDbuff");
    BLinvLbuff=(doublecomplex **)MAlloc((size_t)nblocks*sizeof(doublecomplex *),
					 "ZHERselbinv:BLinvLbuff");
    n_BLLbuff=0;
    for (i=0; i<nblocks; i++) {
        /* extract BL{i} */
        BL_block=mxGetCell(BL,i);
	...
	BLinvDbuff[i]=(doublecomplex *)MAlloc((size_t)k*l*sizeof(doublecomplex),
					       "ZHERselbinv:BLinvDbuff[i]");
	...
	/* 2x2 case, column j starts with row index j */
	   ...
	   prBLD+=j+2;
	   if (piBLD!=NULL) piBLD+=j+2;
	   /* 2x2 block (but no pivoting), remember that indexing for zhetri_
...
#ifdef PRINT_INFO
    mexPrintf("ZHERselbinv: final lower triangular part copied\n");fflush(stdout);
    pzBLinvD=BLinvDbuff[k];
    mexPrintf("        ");
    for (j=0; j<n_size; j++)
        mexPrintf("%8d", ipiv[j]);
	...
	for (j=0; j<n_size; j++)
	    mexPrintf("%16.9le+%16.9lei",
		      pzBLinvD[i+j*n_size].r,pzBLinvD[i+j*n_size].i);
	mexPrintf("\n");fflush(stdout);
    }
#endif

    /* use LAPACK's zhetri_ for matrix inversion given the LDL^H decompositon */
#ifdef PRINT_INFO2
    mexPrintf("k=%4d, call zhetri_\n",k+1);
#endif
    pzBLinvD=BLinvDbuff[k];
    j=0;
    zhetri_("L", &n_size, pzBLinvD, &n_size, ipiv, work, &j,1);
    if (j<0) {
       mexPrintf("the %d-th argument had an illegal value\n",-j);
       mexErrMsgTxt("zhetri_ failed\n");
    }
    if (j>0) {
...
#ifdef PRINT_INFO
    mexPrintf("ZHERselbinv: final inverse lower triangular part computed\n");fflush(stdout);
    pzBLinvD=BLinvDbuff[k];
    mexPrintf("        ");
    for (j=0; j<n_size; j++)
        mexPrintf("%8d", (integer)prBLJ[j]);
    ...
    for (i=0; i<n_size; i++) {
        mexPrintf("%8d", (integer)prBLJ[i]);
	for (j=0; j<n_size; j++)
	    mexPrintf("%16.9le+%16.9lei", pzBLinvD[i+j*n_size].r,pzBLinvD[i+j*n_size].i);
	mexPrintf("\n");fflush(stdout);
    }
#endif
	...
	/* copy data to the strict upper triangular part of row j */
	    ...
	    pz->r= pzBLinvD->r;
	    pz->i=-pzBLinvD->i; /* conjugate! */
	    pzBLinvD++;
	} /* end for i */
    } /* end for j */
#ifdef PRINT_INFO
    mexPrintf("ZHERselbinv: inverse diagonal entries extracted\n");fflush(stdout);
    for (j=0; j<n_size; j++)
        mexPrintf("%16.9le", Dbuffr[sumn+j]);
    mexPrintf("\n");fflush(stdout);
    mexPrintf("ZHERselbinv: final inverse diagonal block computed\n");fflush(stdout);
    pzBLinvD=BLinvDbuff[k];
    mexPrintf("        ");
    for (j=0; j<n_size; j++)
        mexPrintf("%8d", (integer)prBLJ[j]);
    ...
    while (k>=0) {
	  ...
	  */
	  ...
	  while (l<m_size) {
		...
		*/
		if (Ii_cont && Ik_cont && Ji_cont) {
		   ...
		   mexPrintf("];\n");
		   mexPrintf("ZHERselbinv: BLinv{%d}.L(Ik,:) = - BLinv{%d}.L(Ii,Ji)  *BL{%d}.L(%d:%d,:)  + BLinv{%d}.L(Ik,:)\n",k+1,i+1,k+1,l+1,j+1,k+1);
		   r=0; s=0;
		   while (r<m_size && s<mi_size) {
		         if ((integer)prBLI[r]<(integer)prBLinvIi[s])
			    r++;
			    ...
			    for (jj=0; jj<n_size; jj++)
			        mexPrintf("%16.9le+%16.9lei",pzBLinvL[r+m_size*jj].r,pzBLinvL[r+m_size*jj].i);
			    mexPrintf("\n");
			    fflush(stdout);
			    r++; s++;
			 } /* end if-elseif-else */
		   ...
		   /* BLinv{k}.L(l:j,:) = - BLinv{i}.L(Ii,Ji)^H*BL{k}.L(Ik,:)  + BLinv{k}.L(l:j,:)  */
		   if (ii && n_size && t) {
#ifdef PRINT_INFO2
		      mexPrintf("call zgemm_\n");
#endif
		      zgemm_("C","N",&ii,&n_size,&t,
			     &alpha,
			     pzBLinvLi+i_first+mi_size*j_first,&mi_size,
			     pzBLL+k_first,&m_size,
			     &beta,
		   ...
		   mexPrintf("];\n");
		   mexPrintf("ZHERselbinv: BLinv{%d}.L(%d:%d,:) = - BLinv{%d}.L(Ii,Ji)' *BL{%d}.L(Ik,:)  + BLinv{%d}.L(%d:%d,:)\n",k+1,l+1,j+1,i+1,k+1,k+1,l+1,j+1);
		   for (jj=l; jj<=j; jj++) {
		       for (q=0; q<n_size; q++)
			   mexPrintf("%16.9le+%16.9lei",pzBLinvL[jj+m_size*q].r,pzBLinvL[jj+m_size*q].i);
		       mexPrintf("\n");
		       fflush(stdout);
		   }
#endif
		   ...
		   if (ii && n_size) {
		      ...
		      mexPrintf("call zgemm_\n");
#endif
		      zgemm_("N","N",&ii,&n_size,&ii,
			     &alpha,
			     pzBLinvDi+j_first+j_first*ni_size,&ni_size,
			     pzBLL+l,&m_size,
			     &beta,
			     pzBLinvL+l,&m_size,1,1);
		   }
		   ...
		   mexPrintf("];\n");
		   mexPrintf("ZHERselbinv: BLinv{%d}.L(%d:%d,:) = - BLinv{%d}.D(Ji,Ji)  *BL{%d}.L(%d:%d,:)  + BLinv{%d}.L(%d:%d,:)\n",
			     k+1,l+1,j+1,i+1,k+1,l+1,j+1,k+1,l+1,j+1);
		   for (r=l; r<=j; r++) {
		       for (s=0; s<n_size; s++)
			   mexPrintf("%16.9le+%16.9lei",pzBLinvL[r+m_size*s].r,pzBLinvL[r+m_size*s].i);
		       mexPrintf("\n");
		       fflush(stdout);
		   }
#endif
		   ...
		   if (level3_BLAS && t) {
			 ...
			 mexPrintf("];\n");
			 mexPrintf("ZHERselbinv: BLinv{%d}.L(Ii,Ji) cached\n",i+1);fflush(stdout);
			 mexPrintf("        ");
			 r=l; s=0;
			 while (s<ni_size) {
			       if ((integer)prBLinvJi[s]==(integer)prBLI[r]) {
			 ...
			 */
...
#ifdef PRINT_INFO
			 mexPrintf("ZHERselbinv: cached zeros instead of  BLinv{%d}.L(Ik,:)\n",k+1);
			 fflush(stdout);
#endif
		      }
		      else {
			 /* pointer to BLinv{k}.L(Ik,:) and LDC */
			 pz2=pzBLinvL+k_first; q=m_size;
			 alpha.r=-1.0;alpha.i=0.0; beta.r=1.0;beta.i=0.0;
		      } /* end if-else */
			 ...
			 mexPrintf("ZHERselbinv: cached                BLinv{%d}.L(Ii,Ji)  *BL{%d}.L(%d:%d,:)\n",
		                   i+1,k+1,l+1,j+1);
			 ...
			 while (p<m_size && q<mi_size) {
				  ...
				  /* copy current row of pz2 to complex-valued buffer of BLinv{k}.L(Ik,:) */
				  pz=BLinvLbuff[k]+p;
				  pz3=pz2;
			 ...
			 while (p<m_size && q<mi_size) {
				  ...
				  q++;
		                  ...
		                  for (s=0; s<n_size; s++)
			              mexPrintf("%16.9le+%16.9lei",pzBLinvL[p+m_size*s].r,pzBLinvL[p+m_size*s].i);
				  mexPrintf("\n");
				  fflush(stdout);
				  p++; q++;
		               } /* end if-elseif-else */
		      ...
		      while (p<m_size && q<mi_size) {
			    ...
			    if (ii<jj)
			       ...
			       for (ii=0; ii<n_size; ii++,pz+=m_size,pz3+=m_size-(j-l+1)) {
		   ...
		   if (level3_BLAS && t) {
...
#ifdef PRINT_INFO
			 mexPrintf("ZHERselbinv: cached copy of BLinv{%d}.L(Ii,Ji)\nIndex set Ji:\n",i+1);
			 fflush(stdout);
			 r=l; s=0;
			 while (s<ni_size) {
			       if ((integer)prBLinvJi[s]==(integer)prBLI[r]) {
			 ...
			 for (p=0; p<t; p++) {
			     for (q=0; q<j-l+1; q++)
			         mexPrintf("%16.9le+%16.9lei",gemm_buff[p+q*t].r,gemm_buff[p+q*t].i);
			     mexPrintf("\n");
			     fflush(stdout);
			 }
#endif
...
#ifdef PRINT_INFO
			 mexPrintf("ZHERselbinv: cached copy of BL{%d}.L(Ik,:)\nIndex set J:\n",i+1);
			 fflush(stdout);
			 for (q=0; q<n_size; q++)
			 ...
			 for (r=0; r<t; r++) {
			     for (s=0; s<n_size; s++)
			         mexPrintf("%16.9le+%16.9lei",pz2[r+s*t].r,pz2[r+s*t].i);
			     mexPrintf("\n");
			     fflush(stdout);
			 }
#endif
			 ...
			 zgemm_("C","N",&ii,&n_size,&t,
		      ...
		      mexPrintf("];\n");
		      mexPrintf("ZHERselbinv: BLinv{%d}.L(%d:%d,:) = - BLinv{%d}.L(Ii,Ji)^H*BL{%d}.L(Ik,:)  + BLinv{%d}.L(%d:%d,:)\n",k+1,l+1,j+1,i+1,k+1,k+1,l+1,j+1);
		      for (jj=l; jj<=j; jj++) {
			  for (q=0; q<n_size; q++)
			      mexPrintf("%16.9le+%16.9lei",pzBLinvL[jj+m_size*q].r,pzBLinvL[jj+m_size*q].i);
			  mexPrintf("\n");
		      ...
		      /* BLinv{k}.L(l:j,:) -=  BLinv{i}.L(Ii,Ji)^H * BL{k}.L(Ik,:) */
#ifdef PRINT_INFO
		      ...
		      while (p<m_size && q<mi_size) {
			    ...
			    if (ii<jj)
			       ...
			       pz =pzBLL+p;
			       pz3=pzBLinvL+l;
			       /* BLinv{k}.L(l:j,:) -=  BLinv{i}.L(q,Ji)^H * BL{k}.L(p,:) */
			       for (ii=0; ii<n_size; ii++,pz+=m_size,pz3+=m_size-(j-l+1)) {
				   /* BLinv{k}.L(l:j,ii) -=  BLinv{i}.L(q,Ji)^H * BL{k}.L(p,ii) */
				   pz2=pzBLinvLi+q;
				   r=l; s=0;
				   while (s<ni_size) {
				         /* column Ji[s] occurs within I(l:j).
					 ...
					 */
				         if ((integer)prBLinvJi[s]==(integer)prBLI[r]) {
					    /* BLinv{k}.L(r,ii)  -=  BLinv{i}.L(q,s)^H * BL{k}.L(p,ii) */
					    pz3->r -= (pz2->r)*(pz->r)+(pz2->i)*(pz->i); /* conjug! */
					    pz3->i -= (pz2->r)*(pz->i)-(pz2->i)*(pz->r); /* conjug! */
					    pz3++;
					    r++;
					 }
					 s++;
		      ...
		      for (p=l; p<=j; p++) {
			  for (q=0; q<n_size; q++)
			      mexPrintf("%16.9le+%16.9lei",pzBLinvL[p+m_size*q].r,pzBLinvL[p+m_size*q].i);
			  mexPrintf("\n");
			  fflush(stdout);
		      }
#endif
		   ...
		   if (!Ji_cont) {
...
#ifdef PRINT_INFO
		      mexPrintf("ZHERselbinv: cached copy of BLinv{%d}.D(Ji,Ji)\nIndices:\n",i+1);
		      fflush(stdout);
		      p=l; q=0;
		      while (q<ni_size) {
			    if ((integer)prBLinvJi[q]==(integer)prBLI[p]) {
		      ...
		      for (p=0; p<j-l+1; p++) {
			  for (q=0; q<j-l+1; q++)
			      mexPrintf("%16.9le+%16.9lei",gemm_buff[p+q*(j-l+1)].r,gemm_buff[p+q*(j-l+1)].i);
			  mexPrintf("\n");
			  fflush(stdout);
		      }

#endif
		      pz=gemm_buff; p=j-l+1;
		   }
		   else {
		      /* pointer to BLinv{i}.D(Ji,Ji) and LDA */
		      pz=pzBLinvDi+j_first+j_first*ni_size; p=ni_size;
		   } /* end if-else */
...
#ifdef PRINT_INFO
	  mexPrintf("ZHERselbinv: %d-th inverse sub-diagonal block computed\n", k+1);fflush(stdout);
	  mexPrintf("        ");
	  for (j=0; j<n_size; j++)
	  ...
	  for (i=0; i<m_size; i++) {
	      ...
	      for (j=0; j<n_size; j++)
		  mexPrintf("%16.9le+%16.9lei", pzBLinvL[i+j*m_size].r, pzBLinvL[i+j*m_size].i);
	      mexPrintf("\n");fflush(stdout);
	  }
#endif
...
#ifdef PRINT_INFO
	  mexPrintf("ZHERselbinv: inverse lower triangular part computed\n");fflush(stdout);
	  pzBLinvD=BLinvDbuff[k];
	  mexPrintf("        ");
	  for (j=0; j<n_size; j++)
	      mexPrintf("%8d", (integer)prBLJ[j]);
	  ...
	  /* BLinv{k}.D = - BL{k}.L^H *BLinv{k}.L + BLinv{k}.D */
	  /* call level 3 BLAS */
	  alpha.r=-1.0;alpha.i=0.0; beta.r=1.0;beta.i=0.0;
	  ii=j-l+1;
	  if (n_size && m_size) {
	     ...
	     mexPrintf("call zgemm_\n");
#endif
	     zgemm_("C","N",&n_size,&n_size,&m_size,
		    &alpha,
		    pzBLL,&m_size,
		    pzBLinvL,&m_size,
		    &beta,
	  ...
	  for (j=0; j<n_size; j++) {
	      ...
	      /* pointer to BLinv{k}.D(j,j+1) */
	      pz=pzBLinvD+n_size-1;
	      /* complex Hermitian symmetrization of the data from the strict lower/upper triangular part of row j */
	      for (i=j+1; i<n_size; i++,pz+=n_size,pzBLinvD++) {
		  val=(pz->r+pzBLinvD->r)/2.0;
		  pz->r=pzBLinvD->r=val;
		  val=(-pz->i+pzBLinvD->i)/2.0; /* conjug! */
		  pzBLinvD->i=val;
		  pz->i=-val; /* conjug! */
	      } /* end for i */
	  } /* end for j */
#ifdef PRINT_INFO
	  mexPrintf("ZHERselbinv: %d-th inverse diagonal block computed\n", k+1);fflush(stdout);
	  pzBLinvD=BLinvDbuff[k];
	  mexPrintf("        ");
	  for (j=0; j<n_size; j++)
	      mexPrintf("%8d", (integer)prBLJ[j]);
	...
	/* create BLinv{k}.L */
	BLinv_blockL=mxCreateDoubleMatrix((mwSize)m_size,(mwSize)n_size, mxCOMPLEX);
	...
	BLinv_blockD=mxCreateDoubleMatrix((mwSize)n_size,(mwSize)n_size, mxCOMPLEX);
...
#ifdef PRINT_INFO
    mexPrintf("ZHERselbinv: memory released\n");fflush(stdout);
#endif
