...
)
{
    ...
    DILUPACKparam param;
    mxArray *A_input;
    integer *p, *invq, nB=0, nleaves, nleaves_backup,nthreads, *ddist, ddistsize,
            *rangtab,*treetab, dimT;
    ...
    A.nc=A.nr=mrows;
    A.ia=(integer *) MAlloc((size_t)(A.nc+1)*sizeof(integer),"partitionilupackmetisn");
    A.ja=(integer *) MAlloc((size_t)nnz     *sizeof(integer),"partitionilupackmetisn");
    A.a=NULL;
    A.ia[0]=1;
    for (i = 0 ; i < ncols ; i++) {
        A.ia[i+1]=A.ia[i];
    ...
    ddistsize=6*nleaves;
    ddist=(integer *) CAlloc((size_t)ddistsize*6,sizeof(integer),"partitionilupackmetisn");
    p    =(integer *) MAlloc((size_t)A.nc*sizeof(integer),"partitionilupackmetisn");
    invq =(integer *) MAlloc((size_t)A.nc*sizeof(integer),"partitionilupackmetisn");
    ierr=DGNLpartition_metis_n(A, p, invq, nleaves, nthreads, ddist, ddistsize, &param);

    /* extract tree information from the Metis partitioning */
    dimT=nleaves*2;
    /* rangtab describes the start of the j-th node in C-style (starting with zero;
       consistently rangtab[dimT-1] is n (mapping behind the partitioning)
    */
    rangtab=(integer *)CAlloc((size_t)dimT,sizeof(integer),"partitionilupackmetisn:rangtab");
    /* treetab returns the parent information in C-style. There are 2*nleaves-1
       ...
       parent
    */
    treetab=(integer *)CAlloc((size_t)dimT,sizeof(integer),"partitionilupackmetisn:treetab");
