...
void mexFunction(
  ...
  integer *ibuff, *istack, jj, *iconvert;
  mwIndex jstruct, *irs, *jcs;
  const mwSize mydims[] = {1, 1};
  double dbuf, dbufr, dbufi, *A_valuesR, *A_valuesI, *convert, *sr, *si, *pr,
      *pi, det, xr, xi;
  ...
  A.a =
      (doublecomplex *)MAlloc(nnz * sizeof(doublecomplex), "ZHERilupackfactor");
  ...
  */
  ...
  if (tv_exists && tv_field >= 0) {
    ...
    pr = mxGetPr(tmp);
    param->tv = (doublecomplex *)MAlloc((size_t)A.nr * sizeof(doublecomplex),
                                        "ZGNLilupackfactor");
    if (!mxIsComplex(tmp)) {
      for (i = 0; i < A.nr; i++) {
        param->tv[i].r = pr[i];
  ...
  PRE =
      (ZAMGlevelmat *)MAlloc((size_t)sizeof(ZAMGlevelmat), "ZGNLilupackfactor");
  ...
  n = A.nr;
  convert =
      (double *)MAlloc((size_t)n * sizeof(double), "ZGNLilupackfactor:convert");
  iconvert = (integer *)MAlloc((size_t)n * sizeof(integer),
                               "ZGNLilupackfactor:iconvert");
  /* mexPrintf("export options, nlev=%d\n",PRE->nlev);fflush(stdout); */
  for (jstruct = 0; jstruct < PRE->nlev; jstruct++) {
    ...
    if (jstruct == PRE->nlev - 1 &&
      ...
      for (i = 0; i < ((PRE->issingle) ? scurrent->nB : current->nB); i++) {
        ...
        *si++ = 0.0;
        ...
        dbufr = (PRE->issingle) ? scurrent->LU.a[m].r : current->LU.a[m].r;
        dbufi = (PRE->issingle) ? scurrent->LU.a[m].i : current->LU.a[m].i;
        ...
        for (j = i + 1; j < ((PRE->issingle) ? scurrent->nB : current->nB);
          ...
          xr = ((PRE->issingle) ? scurrent->LU.a[m].r : current->LU.a[m].r);
          xi = ((PRE->issingle) ? scurrent->LU.a[m].i : current->LU.a[m].i);
          *sr++ = (xr * dbufr + xi * dbufi) / dbuf;
          *si++ = (-xr * dbufi + xi * dbufr) / dbuf;
        } /* end for j */
      }   /* end for i */
    ...
    if (jstruct == PRE->nlev - 1 &&
      ...
      for (i = 0; i < ((PRE->issingle) ? scurrent->nB : current->nB); i++) {
        ...
        m = i * ((PRE->issingle) ? scurrent->nB : current->nB) + i;
        xr = ((PRE->issingle) ? (double)scurrent->LU.a[m].r
                              : current->LU.a[m].r);
        xi = ((PRE->issingle) ? (double)scurrent->LU.a[m].i
                              : current->LU.a[m].i);
        sr[i] = xr;
        si[i] = xi;
      } /* end for i */
    ...
    if (jstruct < PRE->nlev - 1) {
      ...
      if (param->flags & COARSE_REDUCE) {
        ...
        for (i = 0; i <= ((PRE->issingle) ? scurrent->E.nc : current->E.nc);
             i++)
          jcs[i] = 0;
        /* number of entries per column, shifted by one space */
        if (PRE->issingle) {
    ...
    if (jstruct < PRE->nlev - 1) {
      ...
      if (param->flags & COARSE_REDUCE) {
        ...
        for (i = 0; i <= ((PRE->issingle) ? scurrent->F.nc : current->F.nc);
             i++)
          jcs[i] = 0;
    ...
    if (jstruct == PRE->nlev - 1 &&
      ...
      /* mexPrintf("switched to full-matrix processing\n");fflush(stdout); */
      if (PRE->issingle) {
        for (i = 0; i < n; i++)
