...
)
{
    ...
    double     dbuf, dbufr, dbufi, *A_valuesR, *A_valuesI, *convert, *sr, *si, *pr, *pi, det,xr,xi;
    ...
    */
    ...
    iconvert=(integer *)MAlloc((size_t)n*sizeof(integer),"ZGNLilupackfactor:iconvert");
	...
	if (jstruct==PRE->nlev-1 && 
	   ...
	   for (i=0; i<((PRE->issingle)?scurrent->nB:current->nB); i++) {
	       /* diagonal entry U(i,i) */
	        m=i*((PRE->issingle)?scurrent->nB:current->nB)+i;
		xr=((PRE->issingle)?(double)scurrent->LU.a[m].r:current->LU.a[m].r);
		xi=((PRE->issingle)?(double)scurrent->LU.a[m].i:current->LU.a[m].i);
		sr[i]=xr;
		si[i]=xi;
	   } /* end for i */
