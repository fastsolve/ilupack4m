...
/* ========================================================================== */
/* === ZGNLLDUsol mexFunction =========================================== */
/* ========================================================================== */

/*
    Usage:

    Solve Sl^{-1}P^T (LDU^T) PSr^{-1}z=y, where Sl,Sr are diagonal scaling
   matrice,
    P is a permutation, and LDU^T is a triangular factorization with 1x1 and 2x2
   pivots

    Example:

    % for initializing parameters
    z=ZGNLLDUsol(y,Pvec,Slvec,Srvec,L,iD,UT)
...
void mexFunction(
  ...
  mxArray *L_input, *iD_input, *UT_input, *Pvec_input, *Slvec_input,
      *Srvec_input, *y_input, *z_output;
  integer i, j, k, l, n;
  doubleprecision *pPvecr, *pSlvecr, *pSrvecr, *pzr, *pyr, *dbuffr, xr, yr,
      *pPveci, *pSlveci, *pSrveci, *pzi, *pyi, *dbuffi, xi, yi;
  size_t mrows, ncols;
  double *L_valuesR, *iD_valuesR, *UT_valuesR, *L_valuesI, *iD_valuesI,
      *UT_valuesI;
...
#ifdef PRINT_INFO
  mexPrintf("ZGNLLDUsol: input parameter y imported\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("ZGNLLDUsol: input parameter Pvec imported\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("ZGNLLDUsol: input parameter Slvec imported\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("ZGNLLDUsol: input parameter Srvec imported\n");
  fflush(stdout);
#endif
  ...
  if (mrows != ncols || ncols != n) {
    mexErrMsgTxt("Fifth input must be a square matrix of the same size as the "
                 "other inputs.");
  }
...
#ifdef PRINT_INFO
  mexPrintf("ZGNLLDUsol: input parameter L imported\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("ZGNLLDUsol: input parameter iD imported\n");
  fflush(stdout);
#endif

  /* The seventh input must be a square sparse transposed unit upper triangular
   * matrix.*/
  ...
  if (mrows != ncols || ncols != n) {
    mexErrMsgTxt("Seventh input must be a square matrix of the same size as "
                 "the other inputs.");
  }
...
#ifdef PRINT_INFO
  mexPrintf("ZGNLLDUsol: input parameter UT imported\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("ZGNLLDUsol: output vector and buffer allocated\n");
  fflush(stdout);
#endif
  ...
  if (pyi != NULL) {
    for (i = 0; i < n; i++) {
      ...
      j = pPvecr[i] - 1;
      dbuffr[i] = pSlvecr[i] * pyr[j];
      dbuffi[i] = pSlvecr[i] * pyi[j];
    } /* end for i */
  } else {
    for (i = 0; i < n; i++) {
      /* the input vector counts permutations from 1,...,n */
...
#ifdef PRINT_INFO
  mexPrintf("ZGNLLDUsol: permutation + diagonal scaling done\n");
  fflush(stdout);
#endif
  ...
  if (L_valuesI != NULL) {
    for (i = 0; i < n; i++) {
/* downdate with strict lower triangular part excluding the diagonal part which
 * is 1 */
#ifdef PRINT_CHECK
      if (L_ia[i] >= L_ia[i + 1]) {
      ...
      for (k = L_ia[i] + 1; k < L_ia[i + 1]; k++) {
        /* row index */
        j = L_ja[k];
        dbuffr[j] -= L_valuesR[k] * dbuffr[i] - L_valuesI[k] * dbuffi[i];
        dbuffi[j] -= L_valuesR[k] * dbuffi[i] + L_valuesI[k] * dbuffr[i];
      } /* end for k */
    }   /* end for i */
  } else {
    for (i = 0; i < n; i++) {
 ...
 * is 1 */
      ...
      for (k = L_ia[i] + 1; k < L_ia[i + 1]; k++) {
        /* row index */
        j = L_ja[k];
        dbuffr[j] -= L_valuesR[k] * dbuffr[i];
        dbuffi[j] -= L_valuesR[k] * dbuffi[i];
      } /* end for k */
    }   /* end for i */
  }
#ifdef PRINT_INFO
  mexPrintf("ZGNLLDUsol: forward substitution done\n");
  fflush(stdout);
#endif
  ...
  if (iD_valuesI != NULL) {
    ...
    while (i < n) {
      ...
      if (k - j > 1) {
        ...
        if (l - k > 1) { /* 2x2 pivot with four nonzero elements */
          xr = iD_valuesR[j] * dbuffr[i] - iD_valuesI[j] * dbuffi[i] +
               iD_valuesR[k] * dbuffr[i + 1] - iD_valuesI[k] * dbuffi[i + 1];
          yr = iD_valuesR[j + 1] * dbuffr[i] - iD_valuesI[j + 1] * dbuffi[i] +
               iD_valuesR[k + 1] * dbuffr[i + 1] -
               iD_valuesI[k + 1] * dbuffi[i + 1];
          xi = iD_valuesR[j] * dbuffi[i] + iD_valuesI[j] * dbuffr[i] +
               iD_valuesR[k] * dbuffi[i + 1] + iD_valuesI[k] * dbuffr[i + 1];
          yi = iD_valuesR[j + 1] * dbuffi[i] + iD_valuesI[j + 1] * dbuffr[i] +
               iD_valuesR[k + 1] * dbuffi[i + 1] +
               iD_valuesI[k + 1] * dbuffr[i + 1];
        ...
        if (iD_ja[j] != i) { /* 2x2 pivot with a_{ii}=0 */
          l = iD_ia[i + 2];
          if (l - k > 1) { /* 2x2 pivot with three nonzero elements [   0
                              a_{i,i+1};
                                                                     a_{i+1,i}
                              a_{i+1,i+1}] */
            xr = iD_valuesR[k] * dbuffr[i + 1] - iD_valuesI[k] * dbuffi[i + 1];
            yr = iD_valuesR[j] * dbuffr[i] - iD_valuesI[j] * dbuffi[i] +
                 iD_valuesR[k + 1] * dbuffr[i + 1] -
                 iD_valuesI[k + 1] * dbuffi[i + 1];
            xi = iD_valuesR[k] * dbuffi[i + 1] + iD_valuesI[k] * dbuffr[i + 1];
            yi = iD_valuesR[j] * dbuffi[i] + iD_valuesI[j] * dbuffr[i] +
                 iD_valuesR[k + 1] * dbuffi[i + 1] +
                 iD_valuesI[k + 1] * dbuffr[i + 1];
    ...
    while (i < n) {
      ...
      if (k - j > 1) {
        l = iD_ia[i + 2];
        if (l - k > 1) { /* 2x2 pivot with four nonzero elements */
          xr = iD_valuesR[j] * dbuffr[i] + iD_valuesR[k] * dbuffr[i + 1];
          yr =
              iD_valuesR[j + 1] * dbuffr[i] + iD_valuesR[k + 1] * dbuffr[i + 1];
          xi = iD_valuesR[j] * dbuffi[i] + iD_valuesR[k] * dbuffi[i + 1];
          yi =
              iD_valuesR[j + 1] * dbuffi[i] + iD_valuesR[k + 1] * dbuffi[i + 1];
        } else { /* 2x2 pivot with three nonzero elements [ a_{ii}   a_{i,i+1};
                                                        a_{i+1,i}    0     ] */
          xr = iD_valuesR[j] * dbuffr[i] + iD_valuesR[k] * dbuffr[i + 1];
          yr = iD_valuesR[j + 1] * dbuffr[i];
          xi = iD_valuesR[j] * dbuffi[i] + iD_valuesR[k] * dbuffi[i + 1];
          yi = iD_valuesR[j + 1] * dbuffi[i];
        }
        dbuffr[i] = xr;
        dbuffr[i + 1] = yr;
        dbuffi[i] = xi;
        ...
        if (iD_ja[j] != i) { /* 2x2 pivot with a_{ii}=0 */
          ...
          if (l - k > 1) { /* 2x2 pivot with three nonzero elements [   0
                              a_{i,i+1};
                                                                     a_{i+1,i}
                              a_{i+1,i+1}] */
            xr = iD_valuesR[k] * dbuffr[i + 1];
            yr = iD_valuesR[j] * dbuffr[i] + iD_valuesR[k + 1] * dbuffr[i + 1];
            xi = iD_valuesR[k] * dbuffi[i + 1];
            yi = iD_valuesR[j] * dbuffi[i] + iD_valuesR[k + 1] * dbuffi[i + 1];
          } else { /* 2x2 pivot with two nonzero elements [   0      a_{i,i+1};
                                                           a_{i+1,i}    0     ]
                      */
            xr = iD_valuesR[k] * dbuffr[i + 1];
            yr = iD_valuesR[j] * dbuffr[i];
            xi = iD_valuesR[k] * dbuffi[i + 1];
            yi = iD_valuesR[j] * dbuffi[i];
          }
          dbuffr[i] = xr;
          dbuffr[i + 1] = yr;
          dbuffi[i] = xi;
...
#ifdef PRINT_INFO
  mexPrintf("ZGNLLDUsol: multiplication by block diagonal matrix done\n");
  fflush(stdout);
#endif

  /* back substitution with transposed unit upper triangular matrix U^T */
  if (UT_valuesI != NULL) {
    for (i = n - 1; i >= 0; i--) {
      /* scalar product with strict upper triangular part excluding the diagonal
       * part which is 1 */
      for (k = UT_ia[i] + 1; k < UT_ia[i + 1]; k++) {
        /* row index */
        j = UT_ja[k];
        dbuffr[i] -= UT_valuesR[k] * dbuffr[j] - UT_valuesI[k] * dbuffi[j];
        dbuffi[i] -= UT_valuesR[k] * dbuffi[j] + UT_valuesI[k] * dbuffr[j];
      } /* end for k */
    }   /* end for i */
  } else {
    for (i = n - 1; i >= 0; i--) {
      ...
      for (k = UT_ia[i] + 1; k < UT_ia[i + 1]; k++) {
        /* row index */
        j = UT_ja[k];
        dbuffr[i] -= UT_valuesR[k] * dbuffr[j];
        dbuffi[i] -= UT_valuesR[k] * dbuffi[j];
      } /* end for k */
    }   /* end for i */
  }
#ifdef PRINT_INFO
  mexPrintf("ZGNLLDUsol: backward substitution done\n");
  fflush(stdout);
#endif
  ...
  for (i = 0; i < n; i++) {
    /* the input vector counts permutations from 1,...,n */
    j = pPvecr[i] - 1;
    pzr[j] = pSrvecr[i] * dbuffr[i];
    pzi[j] = pSrvecr[i] * dbuffi[i];
  } /* end for i */
#ifdef PRINT_INFO
  mexPrintf("ZGNLLDUsol: permutation + diagonal scaling done\n");
  fflush(stdout);
