...
void mexFunction(
  ...
  A.ia = (integer *)MAlloc((size_t)(A.nc + 1) * sizeof(integer),
                           "symmwmilupackmetisn");
  A.ja =
      (integer *)MAlloc((size_t)nnz * sizeof(integer), "symmwmilupackmetisn");
  ...
  invq =
      (integer *)MAlloc((size_t)A.nc * sizeof(integer), "symmwmilupackmetisn");
  pcolscale =
      (double *)MAlloc((size_t)A.nc * sizeof(double), "symmwmilupackmetisn");
  prowscale = pcolscale;
...
#else /* MUMPS matching */
  ierr =
      DSYMperm_matching_metis_n(A, prowscale, pcolscale, p, invq, &nB, &param);
#endif
