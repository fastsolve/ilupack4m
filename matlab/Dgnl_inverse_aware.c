/* $Id: Dgnl_inverse_aware.c 802 2015-10-19 16:57:31Z bolle $ */
/* ========================================================================== */
/* === Dgnl_inverse_aware mexFunction =========================================== */
/* ========================================================================== */
...
/*
    ...
    [LL,UUT]=Dgnl_inverse_aware(L,D,U,ndense,transpose)
...
)
{
...
#ifdef PRINT_INFO
    mexPrintf("Dgnl_inverse_aware: input parameter L imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("Dgnl_inverse_aware: input parameter D imported\n");fflush(stdout);
#endif

    /* The fifth input must be a string */
...
#ifdef PRINT_INFO
    mexPrintf("Dgnl_inverse_aware: input parameter transpose imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("Dgnl_inverse_aware: input parameter U imported\n");fflush(stdout);
#endif

    ibuff=(integer *) MAlloc((size_t)n*sizeof(integer),  "Dgnl_inverse_aware:ibuff");
    for (i=0; i<n; i++) {
        ibuff[i]=0;
    }
    if (transposename[0]=='t' || transposename[0]=='T') {
       ...
       UT_ia=(integer *)MAlloc((size_t)(n+1)*sizeof(integer *),"Dgnl_inverse_aware:UT_ia");
       nz=U_ia[n];
       UT_ja=(integer *)MAlloc((size_t)nz*sizeof(integer *),"Dgnl_inverse_aware:UT_ja");
       UT_valuesR=(double *)MAlloc((size_t)nz*sizeof(double *),"Dgnl_inverse_aware:UT_valuesR");
...
#ifdef PRINT_INFO
    mexPrintf("Dgnl_inverse_aware: input parameter ndense imported\n");fflush(stdout);
#endif
    ...
    LL   =(integer **)MAlloc((size_t)n*sizeof(integer *),"Dgnl_inverse_aware:LL");
    nLL  =(integer *) MAlloc((size_t)n*sizeof(integer),  "Dgnl_inverse_aware:nLL");
    /* elbow buffer space */ 
    eLL  =(integer *) MAlloc((size_t)n*sizeof(integer),  "Dgnl_inverse_aware:nLL");
	...
	LL[i]=(integer *) MAlloc((size_t)eLL[i]*sizeof(integer),"Dgnl_inverse_aware:LL[i]");
        /* copy indices */
    ...
    UUT =(integer **)MAlloc((size_t)n*sizeof(integer *),"Dgnl_inverse_aware:UUT");
    nUUT=(integer *) MAlloc((size_t)n*sizeof(integer),  "Dgnl_inverse_aware:nUUT");
    /* elbow buffer space */ 
    eUUT=(integer *) MAlloc((size_t)n*sizeof(integer),  "Dgnl_inverse_aware:eUUT");
	...
	/* provide memory for nonzeros of column i */
	UUT[i]=(integer *) MAlloc((size_t)eUUT[i]*sizeof(integer),"Dgnl_inverse_aware:UUT[i]");
        /* copy indices */
    ...
    plhs[1]=mxCreateSparse((mwSize)n,(mwSize)n, (mwSize)nz, mxREAL);
...
#ifdef PRINT_INFO
    mexPrintf("Dgnl_inverse_aware: memory released\n");fflush(stdout);
#endif
