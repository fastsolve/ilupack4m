...
void mexFunction(
  ...
  integer i, j, k, l, m, n, nz, p, q, r, s, ndense, *ibuff, **LL, *LLi, *LLj,
      *nLL, *eLL, **UUT, *UUTi, *UUTj, *nUUT, *eUUT;
  char *transposename;
  ...
  transposename =
      (char *)mxCalloc((size_t)(mrows * ncols + 1), (size_t)sizeof(char));
  ...
  if (transposename[0] == 't' || transposename[0] == 'T') {
    ...
    UT_ia = (integer *)MAlloc((size_t)(n + 1) * sizeof(integer *),
                              "Dgnl_inverse_aware:UT_ia");
    nz = U_ia[n];
    UT_ja = (integer *)MAlloc((size_t)nz * sizeof(integer *),
                              "Dgnl_inverse_aware:UT_ja");
    UT_valuesR = (double *)MAlloc((size_t)nz * sizeof(double *),
                                  "Dgnl_inverse_aware:UT_valuesR");
  ...
  LL = (integer **)MAlloc((size_t)n * sizeof(integer *),
                          "Dgnl_inverse_aware:LL");
  nLL =
      (integer *)MAlloc((size_t)n * sizeof(integer), "Dgnl_inverse_aware:nLL");
  /* elbow buffer space */
  eLL =
      (integer *)MAlloc((size_t)n * sizeof(integer), "Dgnl_inverse_aware:nLL");
  ...
  UUT = (integer **)MAlloc((size_t)n * sizeof(integer *),
                           "Dgnl_inverse_aware:UUT");
  nUUT =
      (integer *)MAlloc((size_t)n * sizeof(integer), "Dgnl_inverse_aware:nUUT");
  /* elbow buffer space */
  eUUT =
      (integer *)MAlloc((size_t)n * sizeof(integer), "Dgnl_inverse_aware:eUUT");
  ...
  for (i = 0; i < n; i++) {
    ...
    /* provide memory for nonzeros of column i */
    UUT[i] = (integer *)MAlloc((size_t)eUUT[i] * sizeof(integer),
  ...
  while (i < n) {
    ...
    if (D_ia[i + 1] - D_ia[i] > 1) {
      ...
      if (m + 1 > nUUT[i + 1]) {
        ...
        UUT[i + 1] = (integer *)ReAlloc(UUT[i + 1],
                                        (size_t)eUUT[i + 1] * sizeof(integer),
      ...
      if (j >= ndense)
        ...
        if (nUUT[j] < n - j) {
          ...
          if (m + 1 > nUUT[j]) {
            ...
            UUT[j] =
                (integer *)ReAlloc(UUT[j], (size_t)eUUT[j] * sizeof(integer),
