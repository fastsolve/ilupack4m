...
/*
    ...
    simplified QMR for real symmetric matrices and real symmetric
   preconditioning
...
void mexFunction(
  ...
  /* Third input parameter: drain, result from matrix-vector multiplication or
   * preconditioning */
  ...
  if (!init) {
    ...
    ipar[0] = 0;
    /* init with zeros */
    ipar[6] = ipar[7] = ipar[8] = ipar[9] = ipar[10] = ipar[11] = ipar[12] =
        ipar[13] = ipar[14] = ipar[15] = 0;
    fpar[2] = fpar[3] = fpar[4] = fpar[5] = fpar[6] = fpar[7] = fpar[8] =
        fpar[9] = fpar[10] = fpar[11] = fpar[12] = fpar[13] = fpar[14] =
            fpar[15] = 0.0;

    /* provide workspace */
    workspace = (doublecomplex *)MAlloc(6 * (size_t)n * sizeof(doublecomplex),
                                        "DSYMilupacksqmr");
  }
  ...
  /* second left hand side: src for matrix-vector multiplication or
   * preconditioning */
