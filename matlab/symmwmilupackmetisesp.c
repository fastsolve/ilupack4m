/* ========================================================================== */
/* === symmwmilupackmetisesp mexFunction ==================================== */
/* ========================================================================== */

/*
    $Id: symmwmilupackmetisesp.c 809 2016-02-17 10:48:31Z bolle $
    Usage:
...
)
{
    ...
    A.nc=A.nr=mrows;
    A.ia=(integer *)MAlloc((size_t)(A.nc+1)*sizeof(integer),"symmwmilupackmetisesp");
    A.ja=(integer *)MAlloc((size_t)nnz     *sizeof(integer),"symmwmilupackmetisesp");
    A.a =(double *) MAlloc((size_t)nnz     *sizeof(double), "symmwmilupackmetisesp");
    A.ia[0]=1;
    for (i = 0 ; i < ncols ; i++) {
        A.ia[i+1]=A.ia[i];
    ...
    /* import ind */
    param.ind=(integer *) MAlloc((size_t)A.nc*sizeof(integer),"symmwmilupackmetisesp");
    ...
    param.indicator=(integer *)MAlloc((size_t)param.nindicator*sizeof(integer),
				      "symmwmilupackmetisesp:param.indicator");
    j=0;
    k=0;
    lp=lm=0;
    for (i=0; i<A.nc; i++) {
    ...
    p   =(integer *) MAlloc((size_t)A.nc*sizeof(integer),"symmwmilupackmetisesp");
    invq=(integer *) MAlloc((size_t)A.nc*sizeof(integer),"symmwmilupackmetisesp");
    prowscale=(double *) MAlloc((size_t)A.nc*sizeof(double),"symmwmilupackmetisesp");
    pcolscale=prowscale;
#ifdef _MC64_MATCHING_    
    ierr=DSYMperm_mc64_metis_e_sp(A, prowscale, pcolscale, p, invq, &nB, &param);
#elif defined _PARDISO_MATCHING_
    ierr=DSYMperm_mwm_metis_e_sp(A, prowscale, pcolscale, p, invq, &nB, &param);
#else /* MUMPS matching */
    ierr=DSYMperm_matching_metis_e_sp(A, prowscale, pcolscale, p, invq, &nB, &param);
#endif
