...
void mexFunction(
  ...
  A.ia = (integer *)MAlloc((size_t)(A.nc + 1) * sizeof(integer),
                           "symmwmilupackmetisesp");
  A.ja =
      (integer *)MAlloc((size_t)nnz * sizeof(integer), "symmwmilupackmetisesp");
  ...
  /* import ind */
  param.ind = (integer *)MAlloc((size_t)A.nc * sizeof(integer),
                                "symmwmilupackmetisesp");
  ...
  invq = (integer *)MAlloc((size_t)A.nc * sizeof(integer),
                           "symmwmilupackmetisesp");
  prowscale =
      (double *)MAlloc((size_t)A.nc * sizeof(double), "symmwmilupackmetisesp");
  pcolscale = prowscale;
#ifdef _MC64_MATCHING_
  ierr =
      DSYMperm_mc64_metis_e_sp(A, prowscale, pcolscale, p, invq, &nB, &param);
#elif defined _PARDISO_MATCHING_
  ierr = DSYMperm_mwm_metis_e_sp(A, prowscale, pcolscale, p, invq, &nB, &param);
#else /* MUMPS matching */
  ierr = DSYMperm_matching_metis_e_sp(A, prowscale, pcolscale, p, invq, &nB,
                                      &param);
#endif
