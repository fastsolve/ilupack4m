...
)
{
    ...
    work=(doublecomplex *)MAlloc((size_t)n*sizeof(doublecomplex),"ZSYMselbinv:work");
    /* auxiliary buff for output D */
    Dbuffr=(doubleprecision *)MAlloc((size_t)n*sizeof(doubleprecision),"ZSYMselbinv:Dbuffr");
    Dbuffi=(doubleprecision *)MAlloc((size_t)n*sizeof(doubleprecision),"ZSYMselbinv:Dbuffi");
    /* auxiliary buffer for level 3 BLAS */
    size_gemm_buff=n;
    gemm_buff=(doublecomplex *)MAlloc((size_t)size_gemm_buff*sizeof(doublecomplex),"ZSYMselbinv:gemm_buff");
    ...
    while (k>=0) {
	  ...
	  */
	  ...
	  while (l<m_size) {
		...
		*/
		if (Ii_cont && Ik_cont && Ji_cont) {
		   ...
		   mexPrintf("];\n");
		   mexPrintf("ZSYMselbinv: BLinv{%d}.L(Ik,:) = - BLinv{%d}.L(Ii,Ji)  *BL{%d}.L(%d:%d,:)  + BLinv{%d}.L(Ik,:)\n",k+1,i+1,k+1,l+1,j+1,k+1);
		   r=0; s=0;
		   while (r<m_size && s<mi_size) {
		         if ((integer)prBLI[r]<(integer)prBLinvIi[s])
			    r++;
		   ...
		   mexPrintf("];\n");
		   mexPrintf("ZSYMselbinv: BLinv{%d}.L(%d:%d,:) = - BLinv{%d}.L(Ii,Ji)' *BL{%d}.L(Ik,:)  + BLinv{%d}.L(%d:%d,:)\n",k+1,l+1,j+1,i+1,k+1,k+1,l+1,j+1);
		   for (jj=l; jj<=j; jj++) {
		       for (q=0; q<n_size; q++)
		   ...
		   mexPrintf("];\n");
		   mexPrintf("ZSYMselbinv: BLinv{%d}.L(%d:%d,:) = - BLinv{%d}.D(Ji,Ji)  *BL{%d}.L(%d:%d,:)  + BLinv{%d}.L(%d:%d,:)\n",
		   ...
		   if (level3_BLAS && t) {
			 ...
			 */
...
#ifdef PRINT_INFO
			 mexPrintf("ZSYMselbinv: cached zeros instead of  BLinv{%d}.L(Ik,:)\n",k+1);
			 fflush(stdout);
#endif
		      }
		      else {
			 ...
			 mexPrintf("ZSYMselbinv: cached                BLinv{%d}.L(Ii,Ji)  *BL{%d}.L(%d:%d,:)\n",
		                   i+1,k+1,l+1,j+1);
		   ...
		   if (level3_BLAS && t) {
...
#ifdef PRINT_INFO
			 mexPrintf("ZSYMselbinv: cached copy of BL{%d}.L(Ik,:)\nIndex set J:\n",i+1);
			 fflush(stdout);
			 for (q=0; q<n_size; q++)
			 ...
			 zgemm_("T","N",&ii,&n_size,&t,
		      ...
		      mexPrintf("];\n");
		      mexPrintf("ZSYMselbinv: BLinv{%d}.L(%d:%d,:) = - BLinv{%d}.L(Ii,Ji)^T*BL{%d}.L(Ik,:)  + BLinv{%d}.L(%d:%d,:)\n",k+1,l+1,j+1,i+1,k+1,k+1,l+1,j+1);
		      for (jj=l; jj<=j; jj++) {
			  for (q=0; q<n_size; q++)
			      mexPrintf("%16.9le+%16.9lei",pzBLinvL[jj+m_size*q].r,pzBLinvL[jj+m_size*q].i);
			  mexPrintf("\n");
...
#ifdef PRINT_INFO
		      mexPrintf("ZSYMselbinv: cached copy of BLinv{%d}.D(Ji,Ji)\nIndices:\n",i+1);
		      fflush(stdout);
		      p=l; q=0;
		      while (q<ni_size) {
			    if ((integer)prBLinvJi[q]==(integer)prBLI[p]) {
	  ...
	  if (n_size && m_size) {
	     ...
	     mexPrintf("call zgemm_\n");
#endif
	     zgemm_("T","N",&n_size,&n_size,&m_size,
		    &alpha,
		    pzBLL,&m_size,
		    pzBLinvL,&m_size,
		    &beta,
