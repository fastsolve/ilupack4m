...
void mexFunction(
  ...
  A.ia = (integer *)MAlloc((size_t)(A.nc + 1) * sizeof(integer),
                           "symmwmilupackmetise");
  A.ja =
      (integer *)MAlloc((size_t)nnz * sizeof(integer), "symmwmilupackmetise");
  ...
  invq =
      (integer *)MAlloc((size_t)A.nc * sizeof(integer), "symmwmilupackmetise");
  prowscale =
      (double *)MAlloc((size_t)A.nc * sizeof(double), "symmwmilupackmetise");
  pcolscale = prowscale;
...
#else /* MUMPS matching */
  ierr =
      DSYMperm_matching_metis_e(A, prowscale, pcolscale, p, invq, &nB, &param);
#endif
