...
void mexFunction(
  ...
  integer nrhsix, m, *rhsptr = NULL, *rhsind = NULL, ierr, nr, nc, nz, nmax,
                     nzmax, job, ncolumns;
  mwSize nnz, buflen;
  mwIndex *irs, *jcs;
  ...
  A.nc = 0;
  Dreadmtc(&nmax, &nzmax, &job, fname, A.a, A.ja, A.ia, rhs, &ncolumns, rhstyp,
  ...
  if (ncols != 0 && (rhstyp[0] == 'M' || rhstyp[0] == 'm')) {
    rhsptr = (integer *)MAlloc((size_t)(ncols + 1) * sizeof(integer),
                               "Dloadhbo:rhsptr");
    rhsind =
        (integer *)MAlloc((size_t)nrhsix * sizeof(integer), "Dloadhbo:rhsind");
    rhsval =
        (double *)MAlloc((size_t)nrhsix * sizeof(double), "Dloadhbo:rhsval");
  }
  ...
  if (ncols != 0 && (rhstyp[0] == 'M' || rhstyp[0] == 'm'))
    m -= nr * ncols;
  Dreadmtc(&nmax, &nzmax, &job, fname, A.a, A.ja, A.ia, rhs, &m, rhstyp, &nr,
