/* $Id: ZGNLldu2bldu.c 807 2015-11-16 16:21:04Z bolle $ */
/* ========================================================================== */
/* === ZGNLldu2bldu mexFunction ============================================= */
/* ========================================================================== */
...
/*
    ...
    [BL,BD,BUT]=ZGNLldu2bldu(L,D,UT,threshold,maxsize,tol)
...
)
{
    ...
    mxArray         *L_input, *D_input, *UT_input, *threshold_input, *maxsize_input,
                    *tol_input, *block_column, *D_matrix, *L_matrix,
                    *block_index, *BL, *BD, *BUT;
    integer         i,j,k,l,m,kk,ll,n,flag,nnz,p, cnt, cnti, cntj, cntij, BLDsize=10,
                    cntu, cntui, cntuj, cntuij, *idxposu, *idxlstu,
                    *ia, *ja, *idxpos, *idxlst, maxsize, flag_piL, flag_piD, flag_piUT;
    doubleprecision tol, threshold, *prL, *prD, *piL, *piD, mx, valr, vali, nrm, *pr, *pi;
    doublecomplex   *zbuff, *BLDbuff, *pz;
    size_t          mrows, ncols;
    double          *L_valuesR, *D_valuesR, *L_valuesI, *D_valuesI,
                    *UT_valuesR, *UT_valuesI;
    ...
    if (nrhs!=6)
       ...
       mexErrMsgTxt("Sixth input must be a number.");
...
#ifdef PRINT_INFO
    mexPrintf("ZGNLldu2bldu: input parameter L imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZGNLldu2bldu: input parameter D imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZGNLldu2bldu: input parameter UT imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZGNLldu2bldu: input parameter threshold imported\n");fflush(stdout);
#endif
    ...
    if (1!=ncols || mrows!=1 || !mxIsNumeric(prhs[4])) {
       mexErrMsgTxt("Fifth argument must be number.");
    }
    maxsize=*mxGetPr(maxsize_input);
#ifdef PRINT_INFO
    mexPrintf("ZGNLldu2bldu: input parameter maxsize imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZGNLldu2bldu: input parameter tol imported\n");fflush(stdout);
#endif

#ifdef PRINT_INFO
    mexPrintf("ZGNLldu2bldu: input parameters imported\n");fflush(stdout);
#endif


    idxlst =(integer *)MAlloc((size_t)n*sizeof(integer),"ZGNLldu2bldu:idxlst");
    idxpos =(integer *)CAlloc((size_t)n,sizeof(integer),"ZGNLldu2bldu:idxpos");
    idxlstu=(integer *)MAlloc((size_t)n*sizeof(integer),"ZGNLldu2bldu:idxlstu");
    idxposu=(integer *)CAlloc((size_t)n,sizeof(integer),"ZGNLldu2bldu:idxposu");
    zbuff  =(doublecomplex *)MAlloc((size_t)n*sizeof(doublecomplex),"ZGNLldu2bldu:zbuff");
    BLDbuff=(doublecomplex *)MAlloc(((size_t)BLDsize)*BLDsize*sizeof(doublecomplex),"ZGNLldu2bldu:BLDbuff");
    ...
    while (i<n) {
#ifdef PRINT_CHECK
          mexPrintf("ZGNLldu2bldu: analyze columns starting with i=%d\n",i+1);fflush(stdout);
#endif
	  ...
	  cnt=0;
		 ...
		 idxpos[p]=++cnt;
#ifdef PRINT_CHECK
		 mexPrintf("ZGNLldu2bldu: store %d at position %d\n",p+1,cnt);fflush(stdout);
#endif
	      } /* end if */
	  } /* end for m */
	  cnti=cnt;
	      ...
	      valr=UT_valuesR[m];
	      vali=(flag_piUT)? UT_valuesI[m] : 0.0;
	      nrm=sqrt(valr*valr+vali*vali);
	      if (nrm>mx)
		 mx=nrm;
	  } /* end for i */
	  ...
	  while (j<n && flag) {
#ifdef PRINT_CHECK
	        mexPrintf("ZGNLldu2bldu: analyze column j=%d\n",j+1);fflush(stdout);
#endif
		   ...
		   */
		   ...
		   if (cntij <threshold*(cnti+cntj+l) ||
		       cntuij<threshold*(cntui+cntuj+ll)) {
#ifdef PRINT_CHECK
		      mexPrintf("ZGNLldu2bldu: intersection: %d/%d, union: %d/%d, do not merge\n",cntij,cnti+cntj+l,cntuij,cntui+cntuj+ll);fflush(stdout);
#endif
		      ...
		      cntu-=cntuj;
#ifdef PRINT_INFO
		      mexPrintf("ZGNLldu2bldu: additional entries removed\n");fflush(stdout);
#endif
...
#ifdef PRINT_CHECK
		      mexPrintf("ZGNLldu2bldu: intersection: %d/%d, union: %d/%d, merge\n",cntij,cntuij,cnti+cntj+l,cntui+cntuj+ll);fflush(stdout);
#endif
		      if (l) {
	  ...
	  if (j<n-1) {
	     ...
	     if (m>j) {
#ifdef PRINT_CHECK
	        mexPrintf("ZGNLldu2bldu: also add column j=%d\n",j+1);fflush(stdout);
#endif
...
#ifdef PRINT_INFO
	  mexPrintf("ZGNLldu2bldu: create output structures for BL\n");fflush(stdout);
#endif

	  /* set up new block column with four elements BL: J, I, L, D */
...
#ifdef PRINT_CHECK
	  mexPrintf("ZGNLldu2bldu: store column indices\n");fflush(stdout);
#endif
	  for (m=0; m<=j-i; m++) {
	      pr[m]=i+m+1;
#ifdef PRINT_CHECK
...
#ifdef PRINT_INFO
	  mexPrintf("ZGNLldu2bldu: block_column.J set\n");fflush(stdout);
#endif
	  ...
	  for (m=0; m<n; m++) {
	      if (idxpos[m]) {
		 mexPrintf("ZGNLldu2bldu: idxpos[%d]=%d !=0 !!!\n",m+1,idxpos[m]);fflush(stdout);
	    }
	  }
#endif
...
#ifdef PRINT_CHECK
	  mexPrintf("ZGNLldu2bldu: store row indices\n");fflush(stdout);
#endif
	  for (m=0; m<cnt; m++) {
	      l=idxlst[m];
	      pr[m]=l+1;
...
#ifdef PRINT_INFO
	  mexPrintf("ZGNLldu2bldu: block_column.I set\n");fflush(stdout);
#endif

	  /* structure element 2:  BL.L */
	  L_matrix=mxCreateDoubleMatrix((mwSize)cnt,(mwSize)(j-i+1), mxCOMPLEX);
	  prL=mxGetPr(L_matrix);
	  piL=mxGetPi(L_matrix);
	  /* structure element 3:  BL.D */
	  D_matrix=mxCreateDoubleMatrix((mwSize)(j-i+1),(mwSize)(j-i+1), mxCOMPLEX);
	  ...
	  for (m=i; m<=j; m++, prL+=cnt, prD+=j-i+1, piL+=cnt, piD+=j-i+1) {
...
#ifdef PRINT_CHECK
	  mexPrintf("ZGNLldu2bldu: lower triangular diagonal block\n");fflush(stdout);
	  prD=mxGetPr(D_matrix);
	  piD=mxGetPi(D_matrix);
	  for (m=0; m<=j-i; m++) {
	      for (l=0; l<=j-i; l++) {
		mexPrintf("%8.1le+%8.1lei",prD[m+l*(j-i+1)],piD[m+l*(j-i+1)]);
	      }
	      mexPrintf("\n");fflush(stdout);
	  }
#endif
#ifdef PRINT_CHECK
	  mexPrintf("ZGNLldu2bldu: sub-diagonal block\n");fflush(stdout);
	  prL=mxGetPr(L_matrix);
	  piL=mxGetPi(L_matrix);
	  for (m=0; m<cnt; m++) {
	      for (l=0; l<=j-i; l++) {
	  ...
	  if (cnt && j>i) {
	     ...
	     */
	     ...
	     if (BLDsize<m) {
	        BLDsize=m;
		BLDbuff=(doublecomplex *)ReAlloc(BLDbuff,((size_t)BLDsize)*BLDsize*sizeof(doublecomplex),
						 "ZGNLldu2bldu:zbuff");
...
#ifdef PRINT_CHECK
	     mexPrintf("ZGNLldu2bldu: sub-diagonal block after DTRSV\n");fflush(stdout);
	     prL=mxGetPr(L_matrix);
	     piL=mxGetPi(L_matrix);
	     for (m=0; m<cnt; m++) {
	         for (l=0; l<=j-i; l++) {
...
#ifdef PRINT_INFO
	  mexPrintf("ZGNLldu2bldu: block_column.L/D set\n");fflush(stdout);
#endif

	  /* assign block column to cell array BL */
	  mxSetCell(BL,(mwIndex)k,block_column);
#ifdef PRINT_INFO
	  mexPrintf("ZGNLldu2bldu: BL{%d} set\n",k+1);fflush(stdout);
#endif


#ifdef PRINT_INFO
	  mexPrintf("ZGNLldu2bldu: create output structures for BD\n");fflush(stdout);
#endif

	  /* set up new block column with two elements for BD: J, D */
	  block_column=mxCreateStructMatrix((mwSize)1, (mwSize)1, 2, BDnames);

	  /* structure element 0:  BD.J */
	  block_index=mxCreateDoubleMatrix((mwSize)1,(mwSize)(j-i+1), mxREAL);
	  pr=mxGetPr(block_index);
#ifdef PRINT_CHECK
	  mexPrintf("ZGNLldu2bldu: store column indices\n");fflush(stdout);
	  ...
	  /* structure element 1:  BD.D */
	  nnz=D_ia[j+1]-D_ia[i];
	  D_matrix=mxCreateSparse((mwSize)(j-i+1),(mwSize)(j-i+1), (mwSize)nnz, mxCOMPLEX);
...
#ifdef PRINT_INFO
	  mexPrintf("ZGNLldu2bldu: BD{%d}.D set\n",k+1);fflush(stdout);
#endif


#ifdef PRINT_INFO
	  mexPrintf("ZGNLldu2bldu: create output structures for BUT\n");fflush(stdout);
#endif

	  /* set up new block column with four elements BUT: J, I, L, D */
	  block_column=mxCreateStructMatrix((mwSize)1, (mwSize)1, 4, BLnames);
...
#endif
	  /* sort indices of "idxlstu" in increasing order */
	  qqsorti(idxlstu,idxposu,&cntu);
	  /* clear buffer "idxposu" */
	  for (m=0; m<cntu; m++)
	      idxposu[m]=0;
#ifdef PRINT_CHECK
	  for (m=0; m<n; m++) {
	  ...
	  L_matrix=mxCreateDoubleMatrix((mwSize)cntu,(mwSize)(j-i+1), mxCOMPLEX);
	  prL=mxGetPr(L_matrix);
	  piL=mxGetPi(L_matrix);
	  ...
	  for (m=0; m<(j-i+1)*(j-i+1); m++)
	      prD[m]=piD[m]=0.0;
	  /* extract nonzeros from columns i:j */
	  for (m=i; m<=j; m++, prL+=cntu, prD+=j-i+1, piL+=cntu, piD+=j-i+1) {
		  ...
		  */
		     ...
		     piD[p-i]=(flag_piUT)? UT_valuesI[l] : 0.0;
		  }
		  /* index p must be part of UT_{j+1:n,i:j} */
		     ...
		     if (kk) {
		        ...
		        piL[kk-1]=(flag_piUT)? UT_valuesI[l] : 0.0;
		     } /* end if */
		  } /* end if-elseif-elseif */
	      } /* end for l */
	  } /* end for m */
	  ...
	  for (m=0; m<cntu; m++) {
	      for (l=0; l<=j-i; l++) {
		  mexPrintf("%8.1le+%8.1lei",prL[m+l*cntu],piL[m+l*cntu]);
	      }
	      mexPrintf("\n");fflush(stdout);
	  }
#endif
	  ...
	  if (cntu && j>i) {
		...
		BLDbuff=(doublecomplex *)ReAlloc(BLDbuff,((size_t)BLDsize)*BLDsize*sizeof(doublecomplex),
	     ...
	     for (l=0; l<cntu; l++) {
	         /* copy prL/piL into zbuff */
	         for (p=0; p<=j-i; p++) {
		     zbuff[p].r=prL[l+p*cntu];
		     zbuff[p].i=(flag_piUT)? piL[l+p*cntu] : 0.0;
		 } /* end for p */
		 p=1;
	         ztrsv_("L", "T", "U", &m, BLDbuff, &m, zbuff, &p, 1,1,1);
	         /* copy zbuff back to prL/piL */
	         for (p=0; p<=j-i; p++) {
		     prL[l+p*cntu]=zbuff[p].r;
		     if (flag_piUT)
		        piL[l+p*cntu]=zbuff[p].i;
		 } /* end for p */
	     }
#ifdef PRINT_CHECK
	     mexPrintf("ZGNLldu2bldu: sub-diagonal block after DTRSV\n");fflush(stdout);
...
#ifdef PRINT_INFO
	  mexPrintf("ZGNLldu2bldu: BUT{%d} set\n",k+1);fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZGNLldu2bldu: memory released\n");fflush(stdout);
#endif
