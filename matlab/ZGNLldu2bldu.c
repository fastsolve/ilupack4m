...
/*
    ...
    Return cell arrays BL, BD and BUT that refer to a block triangular
   factorization
...
void mexFunction(
  ...
  mxArray *L_input, *D_input, *UT_input, *threshold_input, *maxsize_input,
      *tol_input, *block_column, *D_matrix, *L_matrix, *block_index, *BL, *BD,
      *BUT;
  integer i, j, k, l, m, kk, ll, n, flag, nnz, p, cnt, cnti, cntj, cntij,
      BLDsize = 10, cntu, cntui, cntuj, cntuij, *idxposu, *idxlstu, *ia, *ja,
      *idxpos, *idxlst, maxsize, flag_piL, flag_piD, flag_piUT;
  doubleprecision tol, threshold, *prL, *prD, *piL, *piD, mx, valr, vali, nrm,
      *pr, *pi;
  doublecomplex *zbuff, *BLDbuff, *pz;
  size_t mrows, ncols;
  double *L_valuesR, *D_valuesR, *L_valuesI, *D_valuesI, *UT_valuesR,
      *UT_valuesI;
  ...
  idxlst =
      (integer *)MAlloc((size_t)n * sizeof(integer), "ZGNLldu2bldu:idxlst");
  idxpos = (integer *)CAlloc((size_t)n, sizeof(integer), "ZGNLldu2bldu:idxpos");
  idxlstu =
      (integer *)MAlloc((size_t)n * sizeof(integer), "ZGNLldu2bldu:idxlstu");
  idxposu =
      (integer *)CAlloc((size_t)n, sizeof(integer), "ZGNLldu2bldu:idxposu");
...
#ifdef PRINT_CHECK
          mexPrintf(
              "ZGNLldu2bldu: intersection: %d/%d, union: %d/%d, do not merge\n",
              cntij, cnti + cntj + l, cntuij, cntui + cntuj + ll);
          fflush(stdout);
#endif
...
/* remove index j from the list */
          ...
          mexPrintf("ZGNLldu2bldu: intersection: %d/%d, union: %d/%d, merge\n",
                    cntij, cntuij, cnti + cntj + l, cntui + cntuj + ll);
          fflush(stdout);
#endif
          if (l) {
...
/* transfer sorted indices and store location */
    ...
    L_matrix =
        mxCreateDoubleMatrix((mwSize)cntu, (mwSize)(j - i + 1), mxCOMPLEX);
    prL = mxGetPr(L_matrix);
    piL = mxGetPi(L_matrix);
    ...
    /* extract nonzeros from columns i:j */
    for (m = i; m <= j;
         m++, prL += cntu, prD += j - i + 1, piL += cntu, piD += j - i + 1) {
