...
void mexFunction(
  ...
  memcpy(fname2, pdata,
         (size_t)((sizebuf < 8 * sizeof(char)) ? sizebuf : 8 * sizeof(char)));
  ...
  */
  ...
  rhs = (doublecomplex *)MAlloc((size_t)i * sizeof(doublecomplex),
                                "ZHERsavehbo:rhs");
  sol = rhs;
  tname[0] = ' ';
  tname[1] = ' ';
  ...
  ncolumns = ncols;
  m = nnz;
  Zwritemtc(fname, A.a, A.ja, A.ia, rhs, &ncolumns, tname, &(A.nr), &(A.nc), &m,
            "ILUPACK toolbox for MATLAB. Export routine to Harwell-Boeing "
            "format     ",
            fname2, "cha", strlen(fname), 3, 72, 8, 3);
