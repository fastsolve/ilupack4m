...
)
{
    ...
    work =(doubleprecision *)MAlloc((size_t)n*sizeof(doubleprecision),"DGNLselbinv:work");
    /* auxiliary buff for output D */
    Dbuff=(doubleprecision *)MAlloc((size_t)n*sizeof(doubleprecision),"DGNLselbinv:Dbuff");
    /* auxiliary buffer for level 3 BLAS */
    size_gemm_buff=n;
    gemm_buff=(doubleprecision *)MAlloc((size_t)size_gemm_buff*sizeof(doubleprecision),"DGNLselbinv:gemm_buff");
    ...
    BLinv_blockD=mxCreateSparse((mwSize)n_size,(mwSize)n_size, (mwSize)0, mxREAL);
    /* set each field in BLinv_block structure */
    ...
    BUTinv_blockL=mxCreateDoubleMatrix((mwSize)mut_size,(mwSize)n_size, mxREAL);
    /* set each field in BUTinv_block structure */
    mxSetFieldByNumber(BUTinv_block, (mwIndex)0, 2, BUTinv_blockL);

    /* structure element 3:  D */
    /* create empty sparse n_size x n_size matrix BUTinv{k}.D */
    BUTinv_blockD=mxCreateSparse((mwSize)n_size,(mwSize)n_size, (mwSize)0, mxREAL);
    /* set each field in BUTinv_block structure */
	...
	*pr=*prBDD;
    ...
    while (k>=0) {
	  ...
	  while (l<ml_size) {
		...
		if (Ii_cont && Ik_cont && Ji_cont) {
		   ...
		   if (t && ii)
		      dgemm_("N","N",&t,&n_size,&ii,
			     ...
			     prBLL+l,&ml_size,
			     &beta,
			     prBUTinvL+k_first,&mut_size,1,1);
#ifdef PRINT_INFO
		   mexPrintf("Ik=[");
		   r=0; s=0;
		   ...
		   mexPrintf("];\n");
		   mexPrintf("DGNLselbinv: BUTinv{%d}.L(Ik,:) = - BUTinv{%d}.L(Ii,Ji)  *BL{%d}.L(%d:%d,:)  + BUTinv{%d}.L(Ik,:)\n",k+1,i+1,k+1,l+1,j+1,k+1);
		   r=0; s=0;
		   while (r<mut_size && s<mi_size) {
		         if ((integer)prBUTinvI[r]<(integer)prBUTinvIi[s])
			    r++;
		   ...
		   if (level3_BLAS && t) {
			 ...
			 /* pointer to BUTinv{i}.L(Ii,Ji) and LDA */
		         pr=prBUTinvLi+i_first+mi_size*j_first; p=mi_size;
		      } /* end if-else */
			 ...
			 */
			 alpha=1.0; beta=0.0;
#ifdef PRINT_INFO
			 mexPrintf("DGNLselbinv: cached zeros instead of  BUTinv{%d}.L(Ik,:)\n",k+1);
			 fflush(stdout);
#endif
		      }
		      else {
			 /* pointer to BUTinv{k}.L(Ik,:) and LDC */
		         pr2=prBUTinvL+k_first; q=mut_size;
			 alpha=-1.0; beta=1.0;
		      } /* end if-else */
			 ...
			 mexPrintf("DGNLselbinv: cached                 BUTinv{%d}.L(Ii,Ji)  *BL{%d}.L(%d:%d,:)\n",
		                   i+1,k+1,l+1,j+1);
			 ...
			 while (p<mut_size && q<mi_size) {
		      ...
		      while (p<mut_size && q<mi_size) {
			    ...
			    if (ii<jj)
			       ...
			       for (ii=0; ii<n_size; ii++,pr+=mut_size,pr3+=ml_size-(j-l+1)) {
		...
		*/
		if (Jit_cont && Ikt_cont && Ji_cont) {
		   ...
		   ii=j-l+1;
		   if (tt && ii)
		      dgemm_("N","N",&tt,&n_size,&ii,
			     &alpha,
			     prBDinvDi+jit_first+ni_size*j_first,&ni_size,
			     prBLL+l,&ml_size,
			     &beta,
			     prBUTinvL+kt_first,&mut_size,1,1);
#ifdef PRINT_INFO
		   mexPrintf("Ikt=[");
		   r=0; s=0;
		   ...
		   mexPrintf("];\n");
		   mexPrintf("DGNLselbinv: BUTinv{%d}.L(Ikt,:) = - BDinv{%d}.D(Jit,Ji)  *BL{%d}.L(%d:%d,:)  + BUTinv{%d}.L(Ikt,:)\n",k+1,i+1,k+1,l+1,j+1,k+1);
		   r=0; s=0;
		   while (r<mut_size && s<ni_size) {
		         if ((integer)prBUTinvI[r]<(integer)prBUTinvJi[s])
			    r++;
		   ...
		   if (level3_BLAS && tt) {
			 ...
			 /* pointer to BDinv{i}.D(Jit,Ji) and LDA */
		         pr=prBDinvDi+jit_first+ni_size*j_first; p=ni_size;
		      } /* end if-else */
			 ...
			 */
			 alpha=1.0; beta=0.0;
#ifdef PRINT_INFO
			 mexPrintf("DGNLselbinv: cached zeros instead of  BUTinv{%d}.L(Ikt,:)\n",k+1);
			 fflush(stdout);
#endif
		      }
		      else {
			 /* pointer to BUTinv{k}.L(Ikt,:) and LDC */
		         pr2=prBUTinvL+kt_first; q=mut_size;
			 alpha=-1.0; beta=1.0;
		      } /* end if-else */
			 ...
			 mexPrintf("DGNLselbinv: cached                 BDinv{%d}.D(Jit,Ji)  *BL{%d}.L(%d:%d,:)\n",
		                   i+1,k+1,l+1,j+1);
	  ...
	  while (l<mut_size) {
		...
		*/
		if (Ii_cont && Ik_cont && Ji_cont) {
		   ...
		   mexPrintf("];\n");
		   mexPrintf("DGNLselbinv: BUTinv{%d}.L(%d:%d,:) = - BLinv{%d}.L(Ii,Ji).' *BL{%d}.L(Ik,:)  + BUTinv{%d}.L(%d:%d,:)\n",k+1,l+1,j+1,i+1,k+1,k+1,l+1,j+1);
		   for (jj=l; jj<=j; jj++) {
		       for (q=0; q<n_size; q++)
...
#ifdef PRINT_INFO
			 mexPrintf("DGNLselbinv: cached copy of BL{%d}.L(Ik,:)\nIndex set J:\n",k+1);
			 fflush(stdout);
			 for (q=0; q<n_size; q++)
			 ...
			 dgemm_("T","N",&ii,&n_size,&t,
		      ...
		      while (p<ml_size && q<mi_size) {
			    ...
			    if (ii<jj)
			       ...
			       for (ii=0; ii<n_size; ii++,pr+=ml_size,pr3+=mut_size-(j-l+1)) {
	  ...
	  while (l<mut_size) {
		...
		*/
		if (Ii_cont && Ik_cont && Ji_cont) {
		   ...
		   mexPrintf("];\n");
		   mexPrintf("DGNLselbinv: BLinv{%d}.L(Ik,:) = - BLinv{%d}.L(Ii,Ji)  *BUT{%d}.L(%d:%d,:)  + BLinv{%d}.L(Ik,:)\n",k+1,i+1,k+1,l+1,j+1,k+1);
		   r=0; s=0;
		   while (r<ml_size && s<mi_size) {
		         if ((integer)prBLinvI[r]<(integer)prBLinvIi[s])
			    r++;
		   ...
		   if (level3_BLAS && t) {
			 ...
			 */
			 alpha=1.0; beta=0.0;
#ifdef PRINT_INFO
			 mexPrintf("DGNLselbinv: cached zeros instead of  BLinv{%d}.L(Ik,:)\n",k+1);
			 fflush(stdout);
#endif
		      }
		      else {
			 /* pointer to BLinv{k}.L(Ik,:) and LDC */
		         pr2=prBLinvL+k_first; q=ml_size;
			 alpha=-1.0; beta=1.0;
		      } /* end if-else */
			 ...
			 mexPrintf("DGNLselbinv: cached                BLinv{%d}.L(Ii,Ji)  *BUT{%d}.L(%d:%d,:)\n",
		                   i+1,k+1,l+1,j+1);
	  ...
	  */
	  ...
	  while (l<ml_size) {
		...
		*/
		if (Ii_cont && Ik_cont && Ji_cont) {
		   ...
		   mexPrintf("];\n");
		   mexPrintf("DGNLselbinv: BLinv{%d}.L(%d:%d,:) = - BUTinv{%d}.L(Ii,Ji).' *BUT{%d}.L(Ik,:)  + BLinv{%d}.L(%d:%d,:)\n",k+1,l+1,j+1,i+1,k+1,k+1,l+1,j+1);
		   for (jj=l; jj<=j; jj++) {
		       for (q=0; q<n_size; q++)
...
#ifdef PRINT_INFO
			 mexPrintf("DGNLselbinv: cached copy of BUT{%d}.L(Ik,:)\nIndex set J:\n",k+1);
			 fflush(stdout);
			 for (q=0; q<n_size; q++)
		...
		*/
		if (Jit_cont && Ikt_cont && Ji_cont) {
		   ...
		   ii=j-l+1;
		   if (ii && tt)
		      dgemm_("T","N",&ii,&n_size,&tt,
			     &alpha,
			     prBDinvDi+jit_first+ni_size*j_first,&ni_size,
			     prBUTL+kt_first,&mut_size,
			     &beta,
			     prBLinvL+l,&ml_size,1,1);
#ifdef PRINT_INFO
		   mexPrintf("Ikt=[");
		   ...
		   mexPrintf("];\n");
		   mexPrintf("DGNLselbinv: BLinv{%d}.L(%d:%d,:) = - BDinv{%d}.D(Jit,Ji).' *BUT{%d}.L(Ikt,:)  + BLinv{%d}.L(%d:%d,:)\n",k+1,l+1,j+1,i+1,k+1,k+1,l+1,j+1);
		   for (jj=l; jj<=j; jj++) {
		       for (q=0; q<n_size; q++)
			   mexPrintf("%8.1le",prBLinvL[jj+ml_size*q]);
		       mexPrintf("\n");
		   ...
		   if (level3_BLAS && tt) {
...
#ifdef PRINT_INFO
			 mexPrintf("DGNLselbinv: cached copy of BUT{%d}.L(Ikt,:)\nIndex set J:\n",k+1);
			 fflush(stdout);
			 for (q=0; q<n_size; q++)
			     mexPrintf("%8d",prBUTJ[q]);
			 mexPrintf("\nIndex set Ikt:\n");
			 ...
			 /* pointer to BUT{k}.L(Ikt,:) and LDC */
		         pr2=prBUTL+kt_first; q=mut_size;
		      } /* end if-else */
	  ...
	  mut_size=mxGetN(BUT_blockI)*mxGetM(BUT_blockI);
	  if (mut_size)
	     dgemm_("T","N",&n_size,&n_size,&mut_size,
		    &alpha,
		    prBUTL,&mut_size,
		    prBUTinvL,&mut_size,
		    &beta,
