...
)
{
    ...
    integer        i,j,k,l,m,kk,ll,mm,n,flag,nnz,
                   *ia, *ja, *p, *invp, ierr, *ibuff, flag_piL, flag_piD;
    doubleprecision   *ainvr, *ainvi, Djm1jm1, Djjm1r, Djjm1i, Djj, det, nrm;
    size_t        mrows, ncols;
    double        *Ainv_valuesR, *L_valuesR, *D_valuesR, *p_valuesR, *Delta_valuesR;
    double        *Ainv_valuesI, *L_valuesI, *D_valuesI;
...
#ifdef PRINT_INFO
    mexPrintf("ZHERselinv: input parameter L imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZHERselinv: input parameter D imported\n");fflush(stdout);
#endif
    ...
    if (n!=ncols && mrows!=n) {
       mexErrMsgTxt("Third argument must be a vector of same size as the first matrix.");
    }
    ...
    p    =(integer *)MAlloc((size_t)n*sizeof(integer),"ZHERselinv:p");
    invp =(integer *)MAlloc((size_t)n*sizeof(integer),"ZHERselinv:invp");
    ibuff=(integer *)MAlloc((size_t)n*sizeof(integer),"ZHERselinv:ibuff");
    for (i=0; i<n; i++) {
        j=p_valuesR[i]-1;
        if (j<0 || n<=j) {
...
#ifdef PRINT_INFO
    mexPrintf("ZHERselinv: input parameter p imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("ZHERselinv: input parameter Delta imported\n");fflush(stdout);
#endif

#ifdef PRINT_INFO
    mexPrintf("ZHERselinv: input parameters imported\n");fflush(stdout);
#endif


    /* set up preliminary structure for every column of ainv */
    /* create memory for output matrix Ainv, here: ia */
    ia   =(integer *)MAlloc((size_t)(n+1)  *sizeof(integer),"ZHERselinv:ia");
    i=0;
    ia[0]=0;
...
#ifdef PRINT_INFO
    mexPrintf("ZHERselinv: additional memory for blocked columns: %d\n",nnz);fflush(stdout);
#endif
    nnz+=ia[n];

    ja   =(integer *)MAlloc((size_t)nnz*sizeof(integer),"ZHERselinv:ja");
    ainvr=(double *) MAlloc((size_t)nnz*sizeof(double), "ZHERselinv:ainvr");
    ainvi=(double *) MAlloc((size_t)nnz*sizeof(double), "ZHERselinv:ainvi");
    for (i=0; i<nnz; i++) {
        ainvr[i]=ainvi[i]=0.0;
    } /* end for i */
...
#ifdef PRINT_INFO2
    mexPrintf("ZHERselinv: row index structure inserted\n");fflush(stdout);
    for (i=0; i<n; i++) {
    ...
    if (!flag) {
...
#endif
       mm=ia[j];
       ainvr[mm]=1.0/D_valuesR[m];
       ainvi[mm]=0.0;

       j--;

#ifdef PRINT_INFO
       mexPrintf("ZHERselinv: final inverse lower triangular part computed\n");fflush(stdout);
       mexPrintf("        ");
       mexPrintf("%16d", n);
       mexPrintf("\n");fflush(stdout);
       mexPrintf("%8d", n);
...
#endif
       Djm1jm1=D_valuesR[m];
       Djjm1r =D_valuesR[m+1];
       Djjm1i =(flag_piD)? D_valuesI[m+1] : 0.0;
#ifdef PRINT_CHECK
       if (D_ja[D_ia[j]]!=j-1) { mexPrintf("D: super-diagonal mismatch (%d,%d)\n",D_ja[D_ia[j]]+1,j);fflush(stdout);}
#endif
       m=D_ia[j]+1;
#ifdef PRINT_CHECK
       if (D_ja[m]!=j) { mexPrintf("D: diagonal mismatch (%d,%d)\n",D_ja[m]+1,j+1);fflush(stdout);}
#endif
       Djj    =D_valuesR[m];
       /* determinant for 2x2 matrix inverse */
       det=1.0/(Djm1jm1*Djj-(Djjm1r*Djjm1r+Djjm1i*Djjm1i));
       /* set Ainv(j-1:j,j-1:j) */
       mm=ia[j-1];
#ifdef PRINT_CHECK
       if (ja[mm]!=j-1) { mexPrintf("ainv: mismatch (%d,%d)\n",ja[mm]+1,j);fflush(stdout);}
#endif
#ifdef PRINT_CHECK
       if (ja[mm+1]!=j) { mexPrintf("ainv: mismatch (%d,%d)\n",ja[mm+1]+1,j+1);fflush(stdout);}
#endif
       ainvr[mm]=Djj*det;
       ainvi[mm]=0.0;
       ainvr[mm+1]=-Djjm1r*det;
       ainvi[mm+1]=-Djjm1i*det;
       mm=ia[j];
#ifdef PRINT_CHECK
       if (ja[mm]!=j) { mexPrintf("ainv: mismatch (%d,%d)\n",ja[mm]+1,j+1);fflush(stdout);}
#endif
       ainvr[mm]=Djm1jm1*det;
       ainvi[mm]=0.0;
       ...
       mm=ia[n-2];
       mexPrintf("%8d", n-1);
       mexPrintf("%16.9le+%16.9lei",   ainvr[mm],  ainvi[mm]);
       mexPrintf("%16.9le+%16.9lei\n", ainvr[mm+1],ainvi[mm+1]);
       mm=ia[n-1];
       mexPrintf("%8d", n);
       mexPrintf("%16.9le+%16.9lei",   ainvr[mm],  ainvi[mm]);
       mexPrintf("%16.9le+%16.9lei\n", ainvr[mm+1],ainvi[mm+1]);
	  ...
	  if (!flag) {
...
#ifdef PRINT_CHECK
		 if (ja[ia[j]]!=j) { mexPrintf("Ainv: mismatch (%d,%d)\n",ja[ia[j]]+1,j+1);fflush(stdout);}
#endif
		 l=ia[j]+1; mm=ia[k]; 
		 flag=0;
		 while (l<ia[j+1] && mm<ia[k+1]) {
			  ...
			  if (kk==k) 
			     ...
			     ainvr[l]-=ainvr[mm]*L_valuesR[m]-((flag_piL)? ainvi[mm]*L_valuesI[m] : 0.0);
			     ainvi[l]-=((flag_piL)? ainvr[mm]*L_valuesI[m] : 0.0)+ainvi[mm]*L_valuesR[m];
			     /* mexPrintf("[%d,%d,%d]\n",kk+1,k+1,j+1);fflush(stdout);
				mexPrintf("{%8.1le+%8.1lei,%8.1le+%8.1lei,%8.1le+%8.1lei}\n",ainvr[l],ainvi[l],ainvr[mm],ainvi[mm],L_valuesR[m],L_valuesI[m]);fflush(stdout);
			     */
			  }
			  l++;
			  mm++;
...
#ifdef PRINT_CHECK
		 if (!flag) { mexPrintf("Ainv: row index %d is missing in column %d\n",k+1,j+1);fflush(stdout);}
#endif
		 /* 2. part: strict upper triangular of Ainv(I,k), which is not 
		    stored but contributes to Ainv(k,j). Here use Ainv(I,k)^H */
#ifdef PRINT_CHECK
		 if (L_ja[L_ia[j]]!=j) { mexPrintf("L: mismatch (%d,%d)\n",L_ja[L_ia[j]]+1,j+1);fflush(stdout);}
#endif
		 l=L_ia[j]+1; mm=ia[k]; 
		 while (l<L_ia[j+1] && mm<ia[k+1]) {
			  ...
			  mm++;
		          ...
		          /* Ainv(k,j)=Ainv(k,j)-Ainv(kk,k)^H*L(kk,j) */
			  ainvr[flag]-=ainvr[mm]*L_valuesR[l]+((flag_piL)? ainvi[mm]*L_valuesI[l] : 0.0);
			  ainvi[flag]-=((flag_piL)? ainvr[mm]*L_valuesI[l] : 0.0)-ainvi[mm]*L_valuesR[l];
			  /* mexPrintf("[%d,%d,%d]\n",ja[flag]+1,kk+1,j+1);fflush(stdout);
			     mexPrintf("{%8.1le+%8.1lei,%8.1le+%8.1lei,%8.1le+%8.1lei}\n",ainvr[flag],ainvi[flag],ainvr[mm],ainvi[mm],L_valuesR[l],L_valuesI[l]);fflush(stdout);
			  */
			  l++;
			  mm++;
		       }
	     ...
	     /*********** Compute Ainv(j,j)=1/D(j,j)-L(I,j)^H*Ainv(I,j) ***********/
	     mm=ia[j];
	     m=D_ia[j];
	     ainvr[mm]=1.0/D_valuesR[m];
	     ainvi[mm]=0.0;
#ifdef PRINT_CHECK
	     if (ja[mm]!=D_ja[m]) { mexPrintf("Ainv,D: index mismatch at (%d,%d)\n",ja[mm]+1,D_ja[m]+1);fflush(stdout);}
#endif
#ifdef PRINT_CHECK
	     if (L_ja[L_ia[j]]!=j) { mexPrintf("L: mismatch (%d,%d)\n",L_ja[L_ia[j]]+1,j+1);fflush(stdout);}
#endif
	     ...
	     while (l<ia[j+1] && m<L_ia[j+1]) {
		   ...
		   if (kk<ll)
		      ...
		      /* Ainv(j,j)=Ainv(j,j)-L(kk,j)^H*Ainv(kk,j) */
		      ainvr[mm]-=L_valuesR[m]*ainvr[l]+((flag_piL)? L_valuesI[m]*ainvi[l] : 0.0);
		      /* ainvi[mm]-=L_valuesR[m]*ainvi[l]-L_valuesI[m]*ainvr[l]; */
		      m++;
		      l++;
		   }
	     } /* end while */
	     /******* END Computation Ainv(j,j)=1/D(j,j)-L(I,j)^H*Ainv(I,j) *******/
...
#ifdef PRINT_CHECK
	     if (L_ja[m]<=j) { mexPrintf("L: L(%d,%d)=%8.1le+%8.1lei=0?\n",L_ja[m]+1,j,L_valuesR[m],L_valuesI[m]);fflush(stdout);}
#endif
	     if (L_ja[m]<=j) m++;
	     for (; m<L_ia[j]; m++) {
...
#ifdef PRINT_CHECK
		 if (ja[ia[j-1]]!=j-1 || ja[ia[j-1]+1]!=j) { mexPrintf("Ainv: diagonal or sub-diagonal index mismatch at (%d,%d),(%d,%d)\n",ja[ia[j-1]]+1,j,ja[ia[j-1]+1]+1,j);fflush(stdout);}
#endif
		 l=ia[j-1]+2; mm=ia[k]; 
		 flag=0;
		 ...
		 /* 2. part: strict upper triangular of Ainv(I,k), which is not 
		    stored but contributes to Ainv(k,j-1). Here use Ainv(I,k)^H */
#ifdef PRINT_CHECK
		 if (L_ja[L_ia[j-1]]!=j-1) { mexPrintf("L: diagonal index mismatch at (%d,%d)\n",L_ja[L_ia[j-1]]+1,j);fflush(stdout);}
#endif
		 l=L_ia[j-1]+1;
		 /* skip L(j,j-1) if present (which should be zero) */
#ifdef PRINT_CHECK
		 if (L_ja[l]<=j) { mexPrintf("L: L(%d,%d)=%8.1le+%8.1lei=0??\n",L_ja[l]+1,j,L_valuesR[l],L_valuesI[l]);fflush(stdout);}
#endif
		 if (L_ja[l]<=j) l++;
		 mm=ia[k]; 
		 while (l<L_ia[j] && mm<ia[k+1]) {
			  ...
			  mm++;
		          ...
		          /* Ainv(k,j-1)=Ainv(k,j-1)-Ainv(kk,k)^H*L(kk,j-1) */
			  ainvr[flag]-=ainvr[mm]*L_valuesR[l]+((flag_piL)? ainvi[mm]*L_valuesI[l] : 0.0);
			  ainvi[flag]-=((flag_piL)? ainvr[mm]*L_valuesI[l] : 0.0)-ainvi[mm]*L_valuesR[l];
			  l++;
			  mm++;
...
#ifdef PRINT_CHECK
		 if (ja[ia[j]]!=j) { mexPrintf("Ainv: diagonal index mismatch at (%d,%d)\n",ja[ia[j]]+1,j+1);fflush(stdout);}
#endif
		 l=ia[j]+1; mm=ia[k]; 
		 flag=0;
		 while (l<ia[j+1] && mm<ia[k+1]) {
	     ...
	     /* Compute Ainv(j-1:j,j-1:j)=D(j-1:j,j-1:j)^{-1}-L(I,j-1:j)^H*Ainv(I,j-1:j) */
	     /* extract D(j-1:j,j-1:j) */
	     m=D_ia[j-1];
#ifdef PRINT_CHECK
	     if (D_ja[m]!=j-1) { mexPrintf("D: diagonal mismatch (%d,%d)\n",D_ja[m]+1,j);fflush(stdout);}
       ...
       if (D_ja[D_ia[j]]!=j-1) { mexPrintf("D: super-diagonal mismatch (%d,%d)\n",D_ja[D_ia[j]]+1,j);fflush(stdout);}
	     ...
	     while (l<ia[j] && m<L_ia[j]) {
		   ...
		   if (kk<ll)
		      ...
		      /* Ainv(j-1,j-1)=Ainv(j-1,j-1)-L(kk,j-1)^H*Ainv(kk,j-1) */
		      ainvr[mm]-=L_valuesR[m]*ainvr[l]+((flag_piL)? L_valuesI[m]*ainvi[l] : 0.0);
		      /* ainvi[mm]-=L_valuesR[m]*ainvi[l]-L_valuesI[m]*ainvr[l]; */
		      m++;
		      l++;
	     ...
	     while (l<ia[j] && m<L_ia[j+1]) {
		   ...
		   if (kk<ll)
		      ...
		      /* Ainv(j,j-1)=Ainv(j,j-1)-L(kk,j)^H*Ainv(kk,j-1) */
		      ainvr[mm]-=L_valuesR[m]*ainvr[l]+((flag_piL)? L_valuesI[m]*ainvi[l] : 0.0);
		      ainvi[mm]-=L_valuesR[m]*ainvi[l]-((flag_piL)? L_valuesI[m]*ainvr[l] : 0.0);
		      m++;
		      l++;
		   }
	     } /* end while */
	     ...
	     /* END Computation Ainv(j-1:j,j-1:j)=D(j-1:j,j-1:j)^{-1}-L(I,j-1:j)^H*Ainv(I,j-1:j) */

	     j=j-2;
	  } /* end if-else */
    } /* end while i */
#ifdef PRINT_INFO2
    mexPrintf("ZHERselinv: numerical values structure inserted\n");fflush(stdout);
    for (i=0; i<n; i++) {
        mexPrintf("row indices column %d\n", i+1);
        for (j=ia[i]; j<ia[i+1]; j++) {
	    mexPrintf("%8ld", ja[j]+1);
        ...
        mexPrintf("\n");
        for (j=ia[i]; j<ia[i+1]; j++) {
	    mexPrintf("%8.1le", ainvr[j]);
	}
        mexPrintf("\n");
        for (j=ia[i]; j<ia[i+1]; j++) {
	    mexPrintf("%8.1le", ainvi[j]);
	}
        mexPrintf("\n");
    }
#endif
    ...
    /* 1. step: find out the number of nonzeros per column of the permuted matrix. To do so,
                we count the number of nonzeros in Ainv and check, which ones are in the lower
                triangular part after permutation and which would move to the strict upper 
                triangular part. The latter contribute to the associated row rather than the
                column 
    */
    for (k=0; k<n; k++) {
        i=invp[k];
	...
	for (l=ia[i]; l<ia[i+1]; l++) {
	    ...
	    if (j>=k) 
	       ibuff[k]++;
	    else /* this entry will transfer to the upper triangular part of column k=p(i), thus 
		    it will contribute to the strict lower triangular part of column j instead
		 */
	       ibuff[j]++;
	} /* end for l */
    } /* end for k */

    /* set up pointer structure in advance. This includes the entries which move from one 
	...
	for (l=ia[i]; l<ia[i+1]; l++) {
	    ...
	    if (j>=k) {
	       ...
	       m=Ainv_ia[k]+ibuff[k];
	       Ainv_ja[m]=j;
	       Ainv_valuesR[m]=ainvr[l];
	       Ainv_valuesI[m]=ainvi[l];
	    }
	    else { /* this entry will transfer to the upper triangular part of column k=p(i), thus 
		      its conjugate will contribute to the strict lower triangular part of column j 
		      instead
		   */
	       ibuff[j]--;
	       m=Ainv_ia[j]+ibuff[j];
...
#ifdef PRINT_INFO2
    mexPrintf("ZHERselinv: exported matrix reordered\n");fflush(stdout);
    for (i=0; i<n; i++) {
        mexPrintf("row indices column %d\n", i+1);
        ...
        mexPrintf("\n");
        for (j=Ainv_ia[i]; j<Ainv_ia[i+1]; j++) {
	    mexPrintf("%8.1le", Ainv_valuesI[j]);
	}
        mexPrintf("\n");
    }
#endif
	...
	for (l=Ainv_ia[k]; l<Ainv_ia[k+1]; l++) {
	    ...
	    Ainv_valuesI[l]*=Delta_valuesR[m]*Delta_valuesR[k];
	} /* end for l */
	/* sort column k */
	l=Ainv_ia[k];
	m=Ainv_ia[k+1]-Ainv_ia[k];
	Dqsortpair((doubleprecision *)Ainv_valuesR+l,(doubleprecision *)Ainv_valuesI+l,
		   (integer *)Ainv_ja+l,(integer *)ibuff,&m);
    } /* end for k */
#ifdef PRINT_INFO2
    mexPrintf("ZHERselinv: exported matrix rescaled and indices sorted\n");fflush(stdout);
    for (i=0; i<n; i++) {
        mexPrintf("row indices column %d\n", i+1);
        for (j=Ainv_ia[i]; j<Ainv_ia[i+1]; j++) {
	    mexPrintf("%8ld", Ainv_ja[j]+1);
...
#ifdef PRINT_INFO
    mexPrintf("ZHERselinv: memory released\n");fflush(stdout);
#endif
