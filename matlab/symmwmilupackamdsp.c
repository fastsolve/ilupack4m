...
void mexFunction(
  ...
  /* import ind */
  param.ind =
      (integer *)MAlloc((size_t)A.nc * sizeof(integer), "symmwmilupackamdsp");
  ...
  invq =
      (integer *)MAlloc((size_t)A.nc * sizeof(integer), "symmwmilupackamdsp");
  prowscale =
      (double *)MAlloc((size_t)A.nc * sizeof(double), "symmwmilupackamdsp");
  pcolscale = prowscale;
...
#else /* MUMPS matching */
  ierr =
      DSYMperm_matching_amd_sp(A, prowscale, pcolscale, p, invq, &nB, &param);
#endif
