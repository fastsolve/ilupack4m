...
void mexFunction(
  ...
  DILUPACKparam *param;
  integer n, nnzU, len, k_old, *stack, *Ibuff, ii, blocksize, kk, ll;
  ...
  stack =
      (integer *)MAlloc((size_t)A.nc * sizeof(integer), "DSYMilupackfactor");
  ...
  */
  ...
  for (ifield = 0; ifield < nfields; ifield++) {
    ...
    if (classIDflags[ifield] == mxCHAR_CLASS) {
      ...
      if (!strcmp("elbow", fnames[ifield])) {
      ...
      } else if (!strcmp("isdefinite", fnames[ifield])) {
        if (*mxGetPr(tmp) != 0.0) {
        ...
        param->shifts = (doubleprecision *)MAlloc(
            param->nshifts * sizeof(doubleprecision), "DSYMilupackfactor");
        ...
        */
        ...
        M.ja = (integer *)MAlloc((size_t)nnz * sizeof(integer),
                                 "DSYMilupackfactor");
        M.a =
            (double *)MAlloc((size_t)nnz * sizeof(double), "DSYMilupackfactor");
  ...
  PRE =
      (DAMGlevelmat *)MAlloc((size_t)sizeof(DAMGlevelmat), "DSYMilupackfactor");
  ...
  /* mexPrintf("create preconditioner for output\n"); fflush(stdout); */
  ...
  istack = (integer *)MAlloc((size_t)n * sizeof(integer),
                             "DSYMilupackfactor:istack");
  convert =
      (double *)MAlloc((size_t)n * sizeof(double), "DSYMilupackfactor:convert");
      ...
      if (jstruct == PRE->nlev - 1 &&
        ...
        while (i < ((PRE->issingle) ? scurrent->nB : current->nB)) {
          if (((PRE->issingle) ? scurrent->LU.ia[i] : current->LU.ia[i]) > 0) {
            ...
            while (l--) {
              pr[j++] =
                  ((PRE->issingle) ? scurrent->LU.a[k++] : current->LU.a[k++]);
            }
            i++;
          } else {
            j += i;
        ...
        if (PRE->isblock) {
          ...
          if (param->flags & COARSE_REDUCE) {
            ...
            if (PRE->issingle)
              fout = mxCreateSparse((mwSize)scurrent->n, (mwSize)scurrent->nB,
                                    (mwSize)nnz - scurrent->nB, mxREAL);
            else
              fout = mxCreateSparse((mwSize)current->n, (mwSize)current->nB,
                                    (mwSize)nnz - current->nB, mxREAL);
          }
          /* mexPrintf("nnz=%d\n",nnz-current->nB); */
          ...
          Dbuff = (double *)MAlloc((size_t)2 * m * m * sizeof(double),
                                   "DSYMilupackfactor");
          ...
          for (i = 0; i < ((PRE->issingle) ? scurrent->nB : current->nB);) {
            ...
            for (k = 0; k < blocksize; k++) {
              for (l = 0; l < blocksize; l++) {
                Dbuff[l + k * blocksize] =
                    (PRE->issingle) ? scurrent->LU.a[ii] : current->LU.a[ii];
                ii++;
              }
            }
            ...
            ll = 0;
            dsytrf("u", &blocksize, Dbuff, &blocksize, Ibuff, Dbuff + kk, &kk,
                   &ll, 1);
            if (ll < 0) {
            ...
            kk = blocksize * blocksize;
            dsytri("u", &blocksize, Dbuff, &blocksize, Ibuff, Dbuff + kk, &ll,
                   1);
            if (ll < 0) {
          ...
          for (i = 0; i < ((PRE->issingle) ? scurrent->nB : current->nB);) {
            ...
            if (((PRE->issingle) ? scurrent->LU.ja[scurrent->nB + 1 + i]
                                 : current->LU.ja[current->nB + 1 + i]) == 0) {
              /* column i */
              jcs[i] = k;

              /* diagonal entry */
              irs[k] = i;
              sr[k++] = 1.0 / ((PRE->issingle) ? scurrent->LU.a[i]
                                               : current->LU.a[i]);
              ...
              if (PRE->issingle)
                det = 1.0 / (scurrent->LU.a[i] * scurrent->LU.a[i + 1] -
                             scurrent->LU.a[scurrent->nB + 1 + i] *
                                 scurrent->LU.a[scurrent->nB + 1 + i]);
              else
                det = 1.0 / (current->LU.a[i] * current->LU.a[i + 1] -
                             current->LU.a[current->nB + 1 + i] *
                                 current->LU.a[current->nB + 1 + i]);
        ...
        if (scurrent->isblock)
          fout =
              mxCreateSparse((mwSize)scurrent->nB, (mwSize)scurrent->nB,
                             (mwSize)SPRE->maxblocksize * scurrent->nB, mxREAL);
        else
          fout = mxCreateSparse((mwSize)scurrent->nB, (mwSize)scurrent->nB,
                                (mwSize)2 * scurrent->nB, mxREAL);
      } else {
        if (current->isblock) {
          fout =
              mxCreateSparse((mwSize)current->nB, (mwSize)current->nB,
                             (mwSize)PRE->maxblocksize * current->nB, mxREAL);
          /* mexPrintf("nnz=%d\n",PRE->maxblocksize*current->nB); */
        } else
          fout = mxCreateSparse((mwSize)current->nB, (mwSize)current->nB,
                                (mwSize)2 * current->nB, mxREAL);
      }
      ...
      if (jstruct == PRE->nlev - 1 &&
        ...
        for (i = 0; i < ((PRE->issingle) ? scurrent->nB : current->nB);) {
          ...
          if (((PRE->issingle) ? scurrent->LU.ia[i] : current->LU.ia[i]) > 0) {
            ...
            /* sub-diagonal entry */
            irs[k] = i + 1;
            sr[k++] = ((PRE->issingle) ? scurrent->LU.a[j + 1]
                                       : current->LU.a[j + 1]);
    ...
    if (param->rcomflag == 0) {
      if (jstruct < PRE->nlev - 1) {
        ...
        if (param->flags & COARSE_REDUCE) {
          ...
          if (PRE->issingle)
            fout = mxCreateSparse((mwSize)n - scurrent->nB,
                                  (mwSize)scurrent->nB, (mwSize)nnz, mxREAL);
          else
            fout = mxCreateSparse((mwSize)n - current->nB, (mwSize)current->nB,
                                  (mwSize)nnz, mxREAL);
        }
    ...
    *pr = ((PRE->issingle) ? scurrent->issingle : current->issingle);
      ...
      if (jstruct == PRE->nlev - 2 &&
        ...
        if (PRE->issingle)
          ...
          fout = mxCreateSparse((mwSize)n - current->nB,
                                (mwSize)n - current->nB, (mwSize)0, mxREAL);
      } else {
        if (PRE->issingle)
        ...
        */
        ...
        if (PRE->issingle)
          fout =
              mxCreateSparse((mwSize)scurrent->next->A.nr,
                             (mwSize)scurrent->next->A.nc, (mwSize)nnz, mxREAL);
        else
          fout =
              mxCreateSparse((mwSize)current->next->A.nr,
                             (mwSize)current->next->A.nc, (mwSize)nnz, mxREAL);
        ...
        k = 0;
        for (i = 0;
             i < ((PRE->issingle) ? scurrent->next->A.nr : current->next->A.nr);
             i++) {
          jcs[i] = k;
          k_old = k;
          if (PRE->issingle) {
    ...
    for (j = 0; j < i; j++) {
      if (PRE->issingle)
        ...
        k = current->cntB[j];
      /* create a local vector of length k */
      tmp = mxCreateDoubleMatrix((mwSize)k, (mwSize)k, mxREAL);
