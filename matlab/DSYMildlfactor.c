...
void mexFunction(
    ...
    if (classIDflags[ifield] == mxCHAR_CLASS) {
      ...
      if (!strcmp("ordering", fnames[ifield])) {
        if (strcmp(options.ordering, input_buf)) {
          options.ordering =
              (char *)MAlloc((size_t)buflen * sizeof(char), "DSYMildlfactor");
  ...
  if (options.matching) {
    if (!strcmp("metise", options.ordering)) {
      ...
      if (options.ind == NULL)
        ierr = DSYMperm_mc64_metis_e(A, prowscale, pcolscale, p, invq, &nB,
                                     &options);
      else {
        ierr = DSYMperm_mc64_metis_e_sp(A, prowscale, pcolscale, p, invq, &nB,
                                        &options);
      }
#else
      if (options.ind == NULL)
        ierr = DSYMperm_matching_metis_e(A, prowscale, pcolscale, p, invq, &nB,
                                         &options);
      else
        ierr = DSYMperm_matching_metis_e_sp(A, prowscale, pcolscale, p, invq,
                                            &nB, &options);
#endif
#ifdef PRINT_INFO
      ...
      if (options.ind == NULL)
        ierr =
            DSYMperm_mc64_amd(A, prowscale, pcolscale, p, invq, &nB, &options);
      else {
        ierr = DSYMperm_mc64_amd_sp(A, prowscale, pcolscale, p, invq, &nB,
                                    &options);
      }
#else
      if (options.ind == NULL)
        ierr = DSYMperm_matching_amd(A, prowscale, pcolscale, p, invq, &nB,
                                     &options);
      else {
        ierr = DSYMperm_matching_amd_sp(A, prowscale, pcolscale, p, invq, &nB,
                                        &options);
      }
#endif
#ifdef PRINT_INFO
      ...
      if (options.ind == NULL)
        ierr =
            DSYMperm_mc64_rcm(A, prowscale, pcolscale, p, invq, &nB, &options);
      else {
        ierr = DSYMperm_mc64_rcm_sp(A, prowscale, pcolscale, p, invq, &nB,
                                    &options);
      }
#else
      if (options.ind == NULL)
        ierr = DSYMperm_matching_rcm(A, prowscale, pcolscale, p, invq, &nB,
                                     &options);
      else
        ierr = DSYMperm_matching_rcm_sp(A, prowscale, pcolscale, p, invq, &nB,
                                        &options);
#endif
#ifdef PRINT_INFO
...
#ifdef _MC64_MATCHING_
      if (options.ind == NULL)
        ierr = DSYMperm_mc64_metis_n(A, prowscale, pcolscale, p, invq, &nB,
                                     &options);
      else {
        ierr = DSYMperm_mc64_metis_n_sp(A, prowscale, pcolscale, p, invq, &nB,
                                        &options);
      }
#else
      if (options.ind == NULL)
        ierr = DSYMperm_matching_metis_n(A, prowscale, pcolscale, p, invq, &nB,
                                         &options);
      else {
        ierr = DSYMperm_matching_metis_n_sp(A, prowscale, pcolscale, p, invq,
                                            &nB, &options);
      }
#endif
#ifdef PRINT_INFO
  ...
  /* auxiliary buffers */
  ibuff =
      (integer *)MAlloc((size_t)14 * A.nc * sizeof(integer), "DSYMildlfactor");
  ...
  do {
    ...
    fflush(stdout);
#endif
    DSYMiluc(&(A.nc), A.a, A.ja, A.ia, &(options.lfil), &(options.droptol),
...
#endif
      jlu =
          (integer *)ReAlloc(jlu, mem * sizeof(integer), "DSYMildlfactor:jlu");
