/* ========================================================================== */
/* === symilupackmetisnsp mexFunction ======================================= */
/* ========================================================================== */

/*
    $Id: symmwmilupackmetisnsp.c 809 2016-02-17 10:48:31Z bolle $
    Usage:
...
)
{
    ...
    A.nc=A.nr=mrows;
    A.ia=(integer *)MAlloc((size_t)(A.nc+1)*sizeof(integer),"symmwmilupackmetisnsp");
    A.ja=(integer *)MAlloc((size_t)nnz     *sizeof(integer),"symmwmilupackmetisnsp");
    A.a =(double *) MAlloc((size_t)nnz     *sizeof(double), "symmwmilupackmetisnsp");
    A.ia[0]=1;
    for (i = 0 ; i < ncols ; i++) {
        A.ia[i+1]=A.ia[i];
    ...
    /* import ind */
    param.ind=(integer *) MAlloc((size_t)A.nc*sizeof(integer),"symmwmilupackmetisnsp");
    ...
    param.indicator=(integer *)MAlloc((size_t)param.nindicator*sizeof(integer),
				      "symmwmilupackmetisnsp:param.indicator");
    j=0;
    k=0;
    lp=lm=0;
    for (i=0; i<A.nc; i++) {
    ...
    p   =(integer *) MAlloc((size_t)A.nc*sizeof(integer),"symmwmilupackmetisnsp");
    invq=(integer *) MAlloc((size_t)A.nc*sizeof(integer),"symmwmilupackmetisnsp");
    pcolscale=(double *) MAlloc((size_t)A.nc*sizeof(double),"symmwmilupackmetisnsp");
    prowscale=pcolscale;
#ifdef _MC64_MATCHING_    
    ierr=DSYMperm_mc64_metis_n_sp(A, prowscale, pcolscale, p, invq, &nB, &param);
#elif defined _PARDISO_MATCHING_
    ierr=DSYMperm_mwm_metis_n_sp(A, prowscale, pcolscale, p, invq, &nB, &param);
#else /* MUMPS matching */
    ierr=DSYMperm_matching_metis_n_sp(A, prowscale, pcolscale, p, invq, &nB, &param);
#endif
