/* $Id: ZSYMLDLsol.c 801 2015-10-16 12:30:45Z bolle $ */
/* ========================================================================== */
/* === ZSYMLDLsol mexFunction =========================================== */
/* ========================================================================== */
...
/*
    ...
    P is a permutation, and LDL^T is a triangular factorization with 1x1 and 2x2
   pivots

    Example:

    % for initializing parameters
    z=ZSYMLDLsol(y,Pvec,Svec,L,iD)
...
void mexFunction(
  ...
  integer i, j, k, l, n;
  doubleprecision *pPvecr, *pSvecr, *pzr, *pyr, *dbuffr, xr, yr, *pPveci,
      *pSveci, *pzi, *pyi, *dbuffi, xi, yi;
  size_t mrows, ncols;
  double *L_valuesR, *iD_valuesR, *L_valuesI, *iD_valuesI;
...
#ifdef PRINT_INFO
  mexPrintf("ZSYMLDLsol: input parameter y imported\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("ZSYMLDLsol: input parameter Pvec imported\n");
  fflush(stdout);
#endif
  ...
  pSveci = (double *)mxGetPi(Svec_input);
#ifdef PRINT_INFO
  mexPrintf("ZSYMLDLsol: input parameter Svec imported\n");
  fflush(stdout);
#endif
  ...
  if (mrows != ncols || ncols != n) {
    mexErrMsgTxt("Fourth input must be a square matrix of the same size as the "
                 "other inputs.");
  }
...
#ifdef PRINT_INFO
  mexPrintf("ZSYMLDLsol: input parameter L imported\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("ZSYMLDLsol: input parameter iD imported\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("ZSYMLDLsol: output vector and buffer allocated\n");
  fflush(stdout);
#endif
  ...
  if (pyi != NULL) {
    for (i = 0; i < n; i++) {
      ...
      j = pPvecr[i] - 1;
      dbuffr[i] = pSvecr[i] * pyr[j];
      dbuffi[i] = pSvecr[i] * pyi[j];
    } /* end for i */
  } else {
    for (i = 0; i < n; i++) {
      /* the input vector counts permutations from 1,...,n */
...
#ifdef PRINT_INFO
  mexPrintf("ZSYMLDLsol: permutation + diagonal scaling done\n");
  fflush(stdout);
#endif
 ...
 * is 1 */
...
#ifdef PRINT_INFO
  mexPrintf("ZSYMLDLsol: forward substitution done\n");
  fflush(stdout);
#endif
...
#ifdef PRINT_INFO
  mexPrintf("ZSYMLDLsol: multiplication by block diagonal matrix done\n");
  fflush(stdout);
#endif

  /* back substitution with unit upper triangular matrix U=L^T */
  if (L_valuesI != NULL) {
    for (i = n - 1; i >= 0; i--) {
      ...
      for (k = L_ia[i] + 1; k < L_ia[i + 1]; k++) {
        /* row index */
        j = L_ja[k];
        dbuffr[i] -= L_valuesR[k] * dbuffr[j] - L_valuesI[k] * dbuffi[j];
        dbuffi[i] -= L_valuesR[k] * dbuffi[j] + L_valuesI[k] * dbuffr[j];
      } /* end for k */
    }   /* end for i */
  } else {
    for (i = n - 1; i >= 0; i--) {
...
#ifdef PRINT_INFO
  mexPrintf("ZSYMLDLsol: backward substitution done\n");
  fflush(stdout);
#endif
  ...
  for (i = 0; i < n; i++) {
    /* the input vector counts permutations from 1,...,n */
    j = pPvecr[i] - 1;
    pzr[j] = pSvecr[i] * dbuffr[i];
    pzi[j] = pSvecr[i] * dbuffi[i];
  } /* end for i */
#ifdef PRINT_INFO
  mexPrintf("ZSYMLDLsol: permutation + diagonal scaling done\n");
  fflush(stdout);
