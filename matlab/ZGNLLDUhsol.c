...
void mexFunction(
  ...
  /* forward substitution with conjugate transposed unit upper triangular matrix
   * conj(UT)=U^* */
  if (UT_valuesI != NULL) {
    for (i = 0; i < n; i++) {
 ...
 * is 1 */
      ...
      for (k = UT_ia[i] + 1; k < UT_ia[i + 1]; k++) {
        ...
        dbuffr[j] -= UT_valuesR[k] * dbuffr[i] +
                     UT_valuesI[k] * dbuffi[i]; /* conjugate! */
        dbuffi[j] -= UT_valuesR[k] * dbuffi[i] -
                     UT_valuesI[k] * dbuffr[i]; /* conjugate! */
      }                                         /* end for k */
    }                                           /* end for i */
  } else {
    for (i = 0; i < n; i++) {
 ...
 * is 1 */
  ...
  /* multiplication with a conjugate transposed block diagonal matrix with 1x1
   * and 2x2 pivots */
  if (iD_valuesI != NULL) {
    i = 0;
    j = iD_ia[0];
      ...
      if (k - j > 1) {
        ...
        if (l - k > 1) { /* 2x2 pivot with four nonzero elements */
          xr = iD_valuesR[j] * dbuffr[i] +
               iD_valuesI[j] * dbuffi[i] /* conjugate! */
               + iD_valuesR[j + 1] * dbuffr[i + 1] +
               iD_valuesI[j + 1] * dbuffi[i + 1]; /* conjugate! */
          yr = iD_valuesR[k] * dbuffr[i] +
               iD_valuesI[k] * dbuffi[i] /* conjugate! */
               + iD_valuesR[k + 1] * dbuffr[i + 1] +
               iD_valuesI[k + 1] * dbuffi[i + 1]; /* conjugate! */
          xi = iD_valuesR[j] * dbuffi[i] -
               iD_valuesI[j] * dbuffr[i] /* conjugate! */
               + iD_valuesR[j + 1] * dbuffi[i + 1] -
               iD_valuesI[j + 1] * dbuffr[i + 1]; /* conjugate! */
          yi = iD_valuesR[k] * dbuffi[i] -
               iD_valuesI[k] * dbuffr[i] /* conjugate! */
               + iD_valuesR[k + 1] * dbuffi[i + 1] -
               iD_valuesI[k + 1] * dbuffr[i + 1]; /* conjugate! */
        ...
        if (iD_ja[j] != i) { /* 2x2 pivot with a_{ii}=0 */
          ...
          if (l - k > 1) { /* 2x2 pivot with three nonzero elements [   0
                              a_{i,i+1};
                                                                     a_{i+1,i}
                              a_{i+1,i+1}] */
            xr = iD_valuesR[j] * dbuffr[i + 1] +
                 iD_valuesI[j] * dbuffi[i + 1]; /* conjugate! */
            yr = iD_valuesR[k] * dbuffr[i] +
                 iD_valuesI[k] * dbuffi[i] /* conjugate! */
                 + iD_valuesR[k + 1] * dbuffr[i + 1] +
                 iD_valuesI[k + 1] * dbuffi[i + 1]; /* conjugate! */
            xi = iD_valuesR[j] * dbuffi[i + 1] -
                 iD_valuesI[j] * dbuffr[i + 1]; /* conjugate! */
            yi = iD_valuesR[k] * dbuffi[i] -
                 iD_valuesI[k] * dbuffr[i] /* conjugate! */
                 + iD_valuesR[k + 1] * dbuffi[i + 1] -
                 iD_valuesI[k + 1] * dbuffr[i + 1]; /* conjugate! */
    ...
    while (i < n) {
      ...
      if (k - j > 1) {
        l = iD_ia[i + 2];
        if (l - k > 1) { /* 2x2 pivot with four nonzero elements */
          xr = iD_valuesR[j] * dbuffr[i] + iD_valuesR[j + 1] * dbuffr[i + 1];
          yr = iD_valuesR[k] * dbuffr[i] + iD_valuesR[k + 1] * dbuffr[i + 1];
          xi = iD_valuesR[j] * dbuffi[i] + iD_valuesR[j + 1] * dbuffi[i + 1];
          yi = iD_valuesR[k] * dbuffi[i] + iD_valuesR[k + 1] * dbuffi[i + 1];
        } else { /* 2x2 pivot with three nonzero elements [ a_{ii}   a_{i,i+1};
                                                            a_{i+1,i}    0     ]
                    */
          xr = iD_valuesR[j] * dbuffr[i] + iD_valuesR[j + 1] * dbuffr[i + 1];
  ...
  /* back substitution with conjugate transposed unit lower triangular matrix
   * L^* */
  if (L_valuesI != NULL) {
    for (i = n - 1; i >= 0; i--) {
      ...
      for (k = L_ia[i] + 1; k < L_ia[i + 1]; k++) {
        ...
        dbuffr[i] -= L_valuesR[k] * dbuffr[j] +
                     L_valuesI[k] * dbuffi[j]; /* conjugate! */
        dbuffi[i] -= L_valuesR[k] * dbuffi[j] -
                     L_valuesI[k] * dbuffr[j]; /* conjugate! */
      }                                        /* end for k */
    }                                          /* end for i */
  } else {
    for (i = n - 1; i >= 0; i--) {
