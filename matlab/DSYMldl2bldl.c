/* $Id: DSYMldl2bldl.c 795 2015-08-06 20:20:21Z bolle $ */
/* ========================================================================== */
/* === DSYMldl2bldl mexFunction ============================================= */
/* ========================================================================== */
...
/*
    ...
    [BL,BD]=DSYMldl2bldl(L,D,threshold,maxsize,tol)
...
)
{
    ...
    integer         i,j,k,l,m,kk,n,flag,nnz,p, cnt, cnti, cntj, cntij,
                    *ia, *ja, *idxpos, *idxlst, maxsize;
    doubleprecision tol, threshold, *prL, *prD, mx, val, *pr;
    size_t          mrows, ncols;
    double          *L_valuesR, *D_valuesR;
...
#ifdef PRINT_INFO
    mexPrintf("DSYMldl2bldl: input parameter L imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("DSYMldl2bldl: input parameter threshold imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("DSYMldl2bldl: input parameter maxsize imported\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("DSYMldl2bldl: input parameter tol imported\n");fflush(stdout);
#endif

#ifdef PRINT_INFO
    mexPrintf("DSYMldl2bldl: input parameters imported\n");fflush(stdout);
#endif


    idxlst=(integer *)MAlloc((size_t)n*sizeof(integer),"DSYMldl2bldl:idxlst");
    idxpos=(integer *)CAlloc((size_t)n,sizeof(integer),"DSYMldl2bldl:idxpos");
    ...
    while (i<n) {
#ifdef PRINT_CHECK
          mexPrintf("DSYMldl2bldl: analyze columns starting with i=%d\n",i+1);fflush(stdout);
#endif
	  ...
	  cnt=0;
		 ...
		 idxpos[p]=++cnt;
#ifdef PRINT_CHECK
		 mexPrintf("DSYMldl2bldl: store %d at position %d\n",p+1,cnt);fflush(stdout);
#endif
	      } /* end if */
	  } /* end for m */
	  cnti=cnt;
	  ...
	  while (j<n && flag) {
#ifdef PRINT_CHECK
	        mexPrintf("DSYMldl2bldl: analyze column j=%d\n",j+1);fflush(stdout);
#endif
		   ...
		   */
		   ...
		   if (cntij<threshold*(cnti+cntj+l)) {
#ifdef PRINT_CHECK
		      mexPrintf("DSYMldl2bldl: intersection: %d, union: %d, do not merge\n",cntij,cnti+cntj+l);fflush(stdout);
#endif
		      ...
		      cnt-=cntj;
#ifdef PRINT_INFO
		      mexPrintf("DSYMldl2bldl: additional entries removed\n");fflush(stdout);
#endif
...
#ifdef PRINT_CHECK
		      mexPrintf("DSYMldl2bldl: intersection: %d, union: %d, merge\n",cntij,cnti+cntj+l);fflush(stdout);
#endif
		      if (l) {
	  ...
	  if (j<n-1) {
	     ...
	     if (m>j) {
#ifdef PRINT_CHECK
	        mexPrintf("DSYMldl2bldl: also add column j=%d\n",j+2);fflush(stdout);
#endif
...
#ifdef PRINT_INFO
	  mexPrintf("DSYMldl2bldl: create output structures for BL\n");fflush(stdout);
#endif
...
#ifdef PRINT_CHECK
	  mexPrintf("DSYMldl2bldl: store column indices\n");fflush(stdout);
#endif
	  for (m=0; m<=j-i; m++) {
	      pr[m]=i+m+1;
#ifdef PRINT_CHECK
...
#ifdef PRINT_INFO
	  mexPrintf("DSYMldl2bldl: block_column.J set\n");fflush(stdout);
#endif
	  ...
	  for (m=0; m<n; m++) {
	      if (idxpos[m]) {
		 mexPrintf("DSYMldl2bldl: idxpos[%d]=%d !=0 !!!\n",m+1,idxpos[m]);fflush(stdout);
	    }
	  }
#endif
...
#ifdef PRINT_CHECK
	  mexPrintf("DSYMldl2bldl: store row indices\n");fflush(stdout);
#endif
	  for (m=0; m<cnt; m++) {
	      l=idxlst[m];
	      pr[m]=l+1;
...
#ifdef PRINT_INFO
	  mexPrintf("DSYMldl2bldl: block_column.I set\n");fflush(stdout);
#endif
...
#ifdef PRINT_CHECK
	  mexPrintf("DSYMldl2bldl: lower triangular diagonal block\n");fflush(stdout);
	  prD=mxGetPr(D_matrix);
	  for (m=0; m<=j-i; m++) {
	      for (l=0; l<=j-i; l++) {
...
#endif
#ifdef PRINT_CHECK
	  mexPrintf("DSYMldl2bldl: sub-diagonal block\n");fflush(stdout);
	  prL=mxGetPr(L_matrix);
	  for (m=0; m<cnt; m++) {
	      for (l=0; l<=j-i; l++) {
...
#ifdef PRINT_CHECK
	     mexPrintf("DSYMldl2bldl: sub-diagonal block after DTRSV\n");fflush(stdout);
	     prL=mxGetPr(L_matrix);
	     for (m=0; m<cnt; m++) {
	         for (l=0; l<=j-i; l++) {
		     mexPrintf("%8.1le",prL[m+l*cnt]);
...
#ifdef PRINT_INFO
	  mexPrintf("DSYMldl2bldl: block_column.L/D set\n");fflush(stdout);
#endif

	  /* assign block column to cell array BL */
	  mxSetCell(BL,(mwIndex)k,block_column);
#ifdef PRINT_INFO
	  mexPrintf("DSYMldl2bldl: BL{%d} set\n",k+1);fflush(stdout);
#endif


#ifdef PRINT_INFO
	  mexPrintf("DSYMldl2bldl: create output structures for BD\n");fflush(stdout);
#endif
...
#ifdef PRINT_INFO
	  mexPrintf("DSYMldl2bldl: BD{%d}.D set\n",k+1);fflush(stdout);
#endif
...
#ifdef PRINT_INFO
    mexPrintf("DSYMldl2bldl: memory released\n");fflush(stdout);
#endif
